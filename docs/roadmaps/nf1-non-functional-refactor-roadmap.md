...
| NF1-4 Characterization | In progress. Existing canonical, binding, and L3 coverage is sufficient for planning; add a complete MCP tool-contract manifest first, then the application error compatibility manifest, followed only by seam-local gaps. | Tests fail when a frozen behavior is deliberately perturbed and pass unchanged otherwise. |

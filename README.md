...
当前推荐通过 Codex 插件使用 RouteLedger。当前发布版为 0.10.3，由不可变标签 `routeledger-plugin-v0.10.3` 固定；它统一了 agent-facing operational text，同时保持候选项目内容遵循 `contentLocale`。`main` 是唯一发布干线，`codex-marketplace` 只保留为 0.3.3 的历史锚点分支：
...
- 安装：当前只通过 Codex 插件（Git marketplace）分发；已发布稳定版为 0.10.3；
...
- [Release policy](docs/release/release-policy.md) 与 [0.10.3 release note](docs/release/release-notes/0.10.3.md) — 当前候选基线与发布流程

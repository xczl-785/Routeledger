...
| NF1-2 Recovery decision | Diagnose the Windows authority-lock `EPERM` race, specify persisted ownership and stale-lock recovery as separate functional work, and decide the NF1 storage boundary. | Root tests are green; no recovery-policy change is hidden inside mechanical extraction. |
| NF1-3 Contract inventory | Build a traceability matrix from the five specification areas to docs, schemas, source symbols, and tests. Review the reference SPEC structure only here. | No normative statement without evidence or an explicit gap marker. |
| NF1-4 Characterization | Add tests around facade behavior, error envelopes, registry metadata, canonical serialization, binding checks, and representative L3 transitions. | Tests fail when a frozen behavior is deliberately perturbed and pass unchanged otherwise. |

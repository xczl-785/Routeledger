...
| NF1-1 Test harness | Correct package-local test entry points and resolve the focused MCP version-workflow timeout/process cleanup gap. | Package, focused, and root commands execute real tests and exit cleanly. |
| NF1-2 Contract inventory | Build a traceability matrix from the five specification areas to docs, schemas, source symbols, and tests. Review the reference SPEC structure only here. | No normative statement without evidence or an explicit gap marker. |
| NF1-3 Characterization | Add tests around facade behavior, error envelopes, registry metadata, canonical serialization, binding checks, and representative L3 transitions. | Tests fail when a frozen behavior is deliberately perturbed and pass unchanged otherwise. |
| NF1-4 Recovery decision | Specify persisted ownership and stale-lock recovery as separate functional work and decide the NF1 storage boundary. | No recovery-policy change is hidden inside mechanical extraction. |
...
| NF1-7 Storage boundary | Conditionally replace hidden cross-layer revision coupling only after recovery semantics and an explicit internal envelope design are approved. | Canonical bytes unchanged; concurrency/conflict tests prove the same fail-closed behavior, or the work is explicitly deferred. |

...
export const validateDeferredRouteTarget = (sourceVersion, targetReviewVersionId, knownVersions) => {
    ...
    const targetVersion = knownVersions.find((candidate) => candidate.id === targetId);
...
const addDeferredRouteFailure = (failures, failure, recordId) => {
...
const evaluateConstraintChecks = (version, constraints, checks) => {
    ...
    for (const check of checks) {
        ...
        const scopeMatches = constraint.projectId === version.projectId &&
...
export const evaluateStartGate = ({ targetVersion, currentVersionTodos, dueUndos, requiredDecisionRefs = [], deferredItems = [], constraints = [], constraintChecks = [] }) => {
    const blockers = [];
    ...
    if (dueDeferredIds.length > 0) {
        blockers.push({
            ...
            message: "存在到期 Deferred；启动目标 Version 前请调用 review_deferred 完成评审",
    ...
    const constraintEvaluation = evaluateConstraintChecks(targetVersion, constraints, constraintChecks);
...
export const assertStartGate = (result) => {
    if (!result.allowed) {
...
export const evaluateCloseGate = ({ version, todos, knownTodos = todos, undos, residualAudit, knownVersions, deferredItems = [], constraints = [], constraintChecks = [] }) => {
    ...
    const constraintEvaluation = evaluateConstraintChecks(version, constraints, constraintChecks);
    blockers.push(...constraintEvaluation.blockers);
    return {
        allowed: blockers.length === 0,
        blockers,
...
export const assertCloseGate = (result) => {
    if (!result.allowed) {

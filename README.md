...
当前推荐通过 Codex 插件使用 RouteLedger。当前发布版为 0.10.5，由不可变标签 `routeledger-plugin-v0.10.5` 固定；它保留既有 agent-first 使用方式，同时完成应用服务、存储并发边界和插件运行时的系统化收口。`main` 是唯一发布干线，`codex-marketplace` 只保留为 0.3.3 的历史锚点分支：
...
- 安装：当前只通过 Codex 插件（Git marketplace）分发；已发布稳定版为 0.10.5；
...
- [Release policy](docs/release/release-policy.md) 与 [0.10.5 release note](docs/release/release-notes/0.10.5.md) — 当前候选基线与发布流程

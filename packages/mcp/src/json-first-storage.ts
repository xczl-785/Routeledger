...
import {
  ensureRouteLedgerGitAttributes,
...
export interface RuntimeBindingBlockingIssue {
  kind:
    | "canonical_json_invalid"
    | "json_sqlite_divergence"
    | "multiple_sqlite_projects"
    | "sqlite_read_model_unavailable"
    | "write_in_progress";
  source: "canonical_json" | "sqlite_read_model" | "writer_lock";
  code: string;
  message: string;
...
export interface RuntimeBindingInspection {
  ...
  blockingIssue: RuntimeBindingBlockingIssue | null;
...
export class JsonFirstStorageAdapter implements StoragePort {
  ...
  private toBlockingIssue(options: {
    storageMode: RuntimeBindingStorageMode;
    jsonError: Record<string, unknown> | null;
    sqliteError: Record<string, unknown> | null;
    conflict: Record<string, unknown> | null;
    sqliteState: SqliteState;
    writeLockInfo: RouteLedgerJsonWriteLockInfo | null;
  }): RuntimeBindingBlockingIssue | null {
    if (options.storageMode === "json_invalid" && options.jsonError !== null) {
      return {
        ...
        details:
          options.jsonError.details !== null && typeof options.jsonError.details === "object"
            ? (options.jsonError.details as Record<string, unknown>)
            : null
      };
    }
    if (options.storageMode === "conflict") {
      if (options.conflict !== null) {
        return {
          ...
          details:
            options.conflict.details !== null && typeof options.conflict.details === "object"
              ? (options.conflict.details as Record<string, unknown>)
              : null
        };
      }
    ...
    if (options.storageMode === "write_in_progress") {
      return {
        ...
        details: options.writeLockInfo as unknown as Record<string, unknown> | null
      };
    }
    return null;
  }

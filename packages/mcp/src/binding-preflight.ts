...
export interface RouteLedgerBindingPreflightFailure {
  code: string;
  message: string;
  details: {
    toolName: string;
    ...
    blockedReason: RouteLedgerBindingBlockedReason;
    recommendedNextActions: RouteLedgerRecommendedNextAction[];
    expectedRouteLedgerRoot?: string | null;
    receivedType?: string;
    receivedValue?: unknown;
  };
}

export interface RouteLedgerBindingPreflightResult {
  allowed: boolean;
  failure?: RouteLedgerBindingPreflightFailure;
}

const summarizeReceivedValue = (value: unknown): unknown => {
...
export const getBindingRecommendedNextActions = (
  binding: RouteLedgerBindingSummary
): RouteLedgerRecommendedNextAction[] => {
  const actions: RouteLedgerRecommendedNextAction[] = [
    {
...
export const isBindingToolKindAllowed = (
  binding: RouteLedgerBindingSummary,
  toolKind: RouteLedgerBindingToolKind
): boolean => {
  switch (toolKind) {
    ...
    default: {
      const exhaustiveToolKind: never = toolKind;
      return exhaustiveToolKind;
    }
  }
};

const createBlockedFailure = (
  toolName: string,
  toolKind: RouteLedgerBindingToolKind,
  binding: RouteLedgerBindingSummary,
  blockedReason: RouteLedgerBindingBlockedReason
): RouteLedgerBindingPreflightFailure => ({
...
export const runBindingPreflight = (options: {
  ...
  toolKind: RouteLedgerBindingToolKind;
  binding: RouteLedgerBindingSummary;
  expectedRouteLedgerRoot?: unknown;
}): RouteLedgerBindingPreflightResult => {
  if (
    options.toolKind === "diagnostic" ||
  ...
  const requiresWriteAssertion =
    options.toolKind === "write" || options.toolKind === "bootstrap";

  if (options.toolKind === "bootstrap") {
    if (options.binding.status !== "bound" && options.binding.status !== "uninitialized") {
      return {
        allowed: false,
        failure: createBlockedFailure(
          options.toolName,
          options.toolKind,
          options.binding,
  ...
  if (options.toolKind !== "bootstrap" && options.binding.status !== "bound") {
    const blockedReason =
      options.binding.status === "uninitialized"
        ? {
  ...
  if (
    typeof options.expectedRouteLedgerRoot !== "string" ||
        ...
        details: {
          ...
          expectedRouteLedgerRoot:
            typeof options.expectedRouteLedgerRoot === "string"
              ...
              : null,
          receivedType:
            options.expectedRouteLedgerRoot === null
              ? "null"

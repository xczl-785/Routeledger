...
export interface DeferredConstraintInvariantViolation {
  code: string;
  message: string;
}
...
const hasValidActorShape = (value: unknown): value is Actor =>
  isRecord(value) &&
  isNonBlankString(value.id) &&
  (value.type === "user" || value.type === "agent" || value.type === "system") &&
  (value.displayName === undefined || typeof value.displayName === "string");

export const collectDeferredItemInvariantViolations = (
  value: unknown,
  todos: Todo[] = []
): DeferredConstraintInvariantViolation[] => {
  if (!isRecord(value)) {
    return [
      {
  ...
  const violations: DeferredConstraintInvariantViolation[] = [];
  ...
  if (
    requiredNonBlankFields.some((field) => !isNonBlankString(value[field])) ||
  ...
  const deferred = value as unknown as DeferredItem;
  ...
  if (deferred.status === "pending") {
    ...
    !isNonBlankString(deferred.reviewedAt) ||
    deferred.resolutionOutcome === null ||
  ...
  if (deferred.status === "activated") {
    ...
    if (
      ...
      violations.push({
        ...
        message:
          "activated DeferredItem 必须指向同一 project/work item 且以自身为来源的现有 Todo"
      });
    }
  ...
  if (
    (deferred.resolutionOutcome === "rejected" ||
...
export const collectConstraintInvariantViolations = (
  value: unknown
): DeferredConstraintInvariantViolation[] => {
  if (!isRecord(value)) {
    return [
  ...
  const constraint = value as unknown as Constraint;
...
export const assertDeferredItemInvariant = (
  value: unknown,
  todos: Todo[] = []
): void => {
...
export const assertConstraintInvariant = (value: unknown): void => {

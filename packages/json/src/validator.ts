...
export interface RouteLedgerJsonValidationResult {
  valid: boolean;
  issues: RouteLedgerJsonValidationIssue[];
}

interface CurrentRefSnapshot {
  path: string;
  projectId?: string;
...
const createIssue = (
  severity: RouteLedgerJsonValidationIssue["severity"],
  code: string,
  message: string,
...
): { value?: Record<string, unknown>; issue?: RouteLedgerJsonValidationIssue } => {
  try {
...
const hasValidJsonActorShape = (value: unknown): boolean =>
  isRecord(value) &&
...
const digestsMatch = (
  left: PendingOperation["digest"] | ApprovalArtifact["digest"],
  right: PendingOperation["digest"] | ApprovalArtifact["digest"]
): boolean =>
  ...
  for (const record of records) {
    if (record.projectId !== projectId) {
      issues.push(
        createIssue(
          "error",
          "PROJECT_SCOPE_ID_MISMATCH",
          `${record.kind} 的 project_id 必须等于 root project id`,
          {
            path: record.path,
            details: {
...
const validateVersionReference = (
  issues: RouteLedgerJsonValidationIssue[],
  versionsById: Map<string, Version>,
  version: Version,
  field: "parentVersionId" | "previousVersionId" | "nextVersionId",
  jsonField: "parent_version_id" | "previous_version_id" | "next_version_id",
  code: string
): void => {
  ...
  if (targetId === version.id) {
    issues.push(
      createIssue("error", `${code}_SELF`, `${jsonField} 不能自指向`, {
  ...
  if (!versionsById.has(targetId)) {
    issues.push(
      createIssue("error", code, `${jsonField} 必须指向现有 version`, {
        path: getVersionPath(version.id),
        details: {
          versionId: version.id,
          field: jsonField,
...
const validateVersionChainCycles = (
  issues: RouteLedgerJsonValidationIssue[],
  versions: Version[],
  versionsById: Map<string, Version>,
  field: "parentVersionId" | "previousVersionId" | "nextVersionId",
    ...
    while (current !== undefined) {
      if (visited.has(current.id)) {
        ...
        if (!reportedCycles.has(cycleKey)) {
          ...
          issues.push(
            createIssue("error", code, `${field} 引用链存在循环`, {
              path: getVersionPath(version.id),
              details: {
                versionId: version.id,
...
const createVersionSiblingIssueDetails = (
  version: Version,
...
const validateVersionSiblingStructure = (
  issues: RouteLedgerJsonValidationIssue[],
  versions: Version[],
  versionsById: Map<string, Version>
): void => {
  for (const version of versions) {
    const previousVersion =
      version.previousVersionId === null ? undefined : versionsById.get(version.previousVersionId);
    ...
    if (previousVersion !== undefined) {
      if (previousVersion.projectId !== version.projectId) {
        issues.push(
          createIssue(
            "error",
            "VERSION_SIBLING_PROJECT_MISMATCH",
            "Version sibling 必须属于同一 project",
            {
              path: getVersionPath(version.id),
              details: {
      ...
      if (previousVersion.parentVersionId !== version.parentVersionId) {
        issues.push(
          createIssue(
            "error",
            "VERSION_SIBLING_PARENT_MISMATCH",
            "Version sibling 必须属于同一 parent group",
            {
              path: getVersionPath(version.id),
              details: {
                ...createVersionSiblingIssueDetails(version, previousVersion),
      ...
      if (previousVersion.nextVersionId !== version.id) {
        issues.push(
          createIssue(
            "error",
            "VERSION_SIBLING_PREVIOUS_MISMATCH",
            "previous_version_id 与上游 sibling 的 next_version_id 必须双向一致",
            {
              path: getVersionPath(version.id),
              details: {
                versionId: version.id,
      ...
      if (nextVersion.previousVersionId !== version.id) {
        issues.push(
          createIssue(
            ...
            "next_version_id 与下游 sibling 的 previous_version_id 必须双向一致",
            {
              path: getVersionPath(version.id),
              details: {
                versionId: version.id,
  ...
  const siblingGroupsByProject = new Map<string, Map<string | null, Version[]>>();

  for (const version of versions) {
    let siblingGroupsByParent = siblingGroupsByProject.get(version.projectId);

    if (siblingGroupsByParent === undefined) {
      siblingGroupsByParent = new Map<string | null, Version[]>();
    ...
    for (const group of siblingGroupsByParent.values()) {
      ...
      if (heads.length !== 1) {
        issues.push(
          createIssue(
            ...
            "同一 parent group 必须且只能有一个 sibling head",
            {
      ...
      if (tails.length !== 1) {
        issues.push(
          createIssue(
            "error",
            "VERSION_SIBLING_TAIL_COUNT_INVALID",
            "同一 parent group 必须且只能有一个 sibling tail",
            {
              path: getVersionPath(representative.id),
              details: {
                projectId: representative.projectId,
      ...
      if (visited.size !== group.length) {
        issues.push(
          createIssue(
            "error",
            "VERSION_SIBLING_CHAIN_DISCONNECTED",
            "Version sibling 链必须覆盖同一 parent group 下的全部节点",
            {
              path: getVersionPath(representative.id),
              details: {
                projectId: representative.projectId,
...
const validateNewEntityRawShape = (
  kind: "deferred_item" | "constraint",
  ...
  if (kind === "deferred_item") {
    ...
    return (
      requiredNonBlankFields.every((field) => isNonBlankString(record[field])) &&
...
const validateDocumentContract = (
  ...
  if (contract.kind !== undefined && record.kind !== contract.kind) {
    issues.push(
      createIssue(
        ...
        "JSON 文档 kind 与 canonical 路径类型不匹配",
        {
          path: documentPath,
          details: {
  ...
  if (
    ...
    issues.push(
      createIssue(
        "error",
        "JSON_DOCUMENT_PATH_INVALID",
        "JSON 文档路径必须由 kind 与 id 唯一确定",
        {
          path: documentPath,
          details: {
            kind: contract.kind,
  ...
  if (
    (contract.kind === "deferred_item" || contract.kind === "constraint") &&
    !validateNewEntityRawShape(contract.kind, record)
  ) {
    issues.push(
      createIssue(
        "error",
        "JSON_DOCUMENT_SHAPE_INVALID",
        `${contract.kind} JSON 文档字段类型、必填值或枚举无效`,
        {
          path: documentPath,
          details: {
            kind: contract.kind
...
const isUnsafeAbsolutePath = (value: string): boolean =>
...
const validateAssetRelativePath = (
  issues: RouteLedgerJsonValidationIssue[],
  assetId: string,
  documentPath: string,
  value: string,
  ...
  if (isUnsafeRelativePath(value)) {
    issues.push(
      createIssue("error", "ASSET_RELATIVE_PATH_INVALID", `${field} 必须是 project_root 下的非空相对路径`, {
        path: documentPath,
        details: {
          assetId,
          field,
...
const validateTransitionTarget = (
  ...
  undosById: Map<string, Undo>,
  deferredItemsById: Map<string, DeferredItem>,
  constraintsById: Map<string, Constraint>,
  assetsById: Map<string, Asset>,
  pendingOperationsById: Map<string, PendingOperation>,
  approvalArtifactsById: Map<string, ApprovalArtifact>
): void => {
  const exists =
    (event.targetType === "project" && event.targetId === projectId) ||
  ...
  if (!exists) {
    issues.push(
      createIssue("error", "TRANSITION_EVENT_TARGET_NOT_FOUND", "TransitionEvent target 必须指向现有对象", {
...
export const validateProjectAggregateSnapshot = (
  snapshot: ProjectAggregateSnapshot,
  options: {
    currentRef?: CurrentRefSnapshot;
    legacyAuditArtifactIds?: ReadonlySet<string>;
  } = {}
): RouteLedgerJsonValidationResult => {
  const issues: RouteLedgerJsonValidationIssue[] = [];
  const rawSnapshot = snapshot as unknown;
  ...
  if (!isRecord(rawSnapshot) || !isRecord(rawSnapshot.project)) {
    return {
      ...
      issues: [
        createIssue(
          "error",
          "AGGREGATE_SHAPE_INVALID",
          "ProjectAggregateSnapshot 顶层与 project 必须是对象"
        )
      ]
    };
  }
  ...
  for (const collectionName of collectionNames) {
    if (!Array.isArray(rawSnapshot[collectionName])) {
      issues.push(
        createIssue(
          "error",
          "AGGREGATE_COLLECTION_INVALID",
          `ProjectAggregateSnapshot.${collectionName} 必须是数组`,
          {
            details: {
    ...
    return {
      ...
      issues: [
        createIssue(
          "error",
          "AGGREGATE_SHAPE_INVALID",
          "ProjectAggregateSnapshot.project 的关键 id 字段无效",
          {
  ...
  for (const [collectionName, entities] of legacyEntityCollections) {
    for (const entity of entities as readonly unknown[]) {
      if (!isRecord(entity) || !isNonBlankString(entity.id)) {
        issues.push(
          createIssue(
            "error",
            "AGGREGATE_ENTITY_ID_INVALID",
            `ProjectAggregateSnapshot.${collectionName} 中的实体 id 必须是非空字符串`,
            {
              details: {
                collection: collectionName
              }
  ...
  const deferredItemsById = new Map(
    snapshot.deferredItems.map((deferredItem) => [deferredItem.id, deferredItem])
  );
  const constraintsById = new Map(
    snapshot.constraints.map((constraint) => [constraint.id, constraint])
  );
  const assetsById = new Map(snapshot.assets.map((asset) => [asset.id, asset]));
  const pendingOperationsById = new Map(
    snapshot.pendingOperations.map((operation) => [operation.id, operation])
  );
  ...
  if (project.currentVersionId === null && snapshot.versions.length > 0) {
    ...
    issues.push(
      createIssue(
        ...
        "Project.current_version_id 必须指向现有 version",
        {
          path: PROJECT_DOCUMENT_PATH,
          details: {
            projectId: project.id,
  ...
  if (project.initialVersionId !== null && !versionsById.has(project.initialVersionId)) {
    issues.push(
      createIssue(
        "error",
        "PROJECT_INITIAL_VERSION_NOT_FOUND",
        "Project.initial_version_id 必须指向现有 version",
        {
          path: PROJECT_DOCUMENT_PATH,
          details: {
            projectId: project.id,
  ...
  if (options.currentRef !== undefined) {
    if (options.currentRef.projectId !== project.id) {
      issues.push(
        createIssue(
          "error",
          "CURRENT_REF_PROJECT_MISMATCH",
          "refs/current.json 的 project_id 必须与 project.json 一致",
          {
    ...
    if (options.currentRef.currentVersionId !== project.currentVersionId) {
      issues.push(
        createIssue(
          "error",
          "CURRENT_REF_OUT_OF_SYNC",
          "refs/current.json 必须镜像 project.current_version_id",
          {
            path: options.currentRef.path,
            details: {
  ...
  for (const version of snapshot.versions) {
    const expectedIsCurrent =
      project.currentVersionId !== null && version.id === project.currentVersionId;
    ...
    if (version.isCurrent !== expectedIsCurrent) {
      issues.push(
        createIssue(
          ...
          "Version.is_current 是镜像字段，必须与 project.current_version_id 一致",
          {
            path: getVersionPath(version.id),
            details: {
              versionId: version.id,
    ...
    validateVersionReference(
      ...
      "nextVersionId",
      "next_version_id",
      "VERSION_NEXT_NOT_FOUND"
    );
  }

  validateVersionChainCycles(
    issues,
    snapshot.versions,
    versionsById,
    "parentVersionId",
    "VERSION_PARENT_CYCLE"
  );
  validateVersionChainCycles(
    issues,
    snapshot.versions,
  ...
  for (const workItem of snapshot.workItems) {
    try {
      ...
      issues.push(
        createIssue("error", "WORK_ITEM_ACTIVE_INVALID", "WorkItem active 指针语义无效", {
  ...
  for (const todo of snapshot.todos) {
    if (!workItemsById.has(todo.workItemId)) {
      issues.push(
        createIssue("error", "TODO_WORK_ITEM_NOT_FOUND", "Todo.work_item_id 必须指向现有 WorkItem", {
    ...
    if (!versionsById.has(todo.versionId)) {
      issues.push(
        createIssue("error", "TODO_VERSION_NOT_FOUND", "Todo.version_id 必须指向现有 version", {
          path: getTodoPath(todo.id),
          details: {
            todoId: todo.id,
  ...
  for (const undo of snapshot.undos) {
    if (!workItemsById.has(undo.workItemId)) {
      issues.push(
        createIssue("error", "UNDO_WORK_ITEM_NOT_FOUND", "Undo.work_item_id 必须指向现有 WorkItem", {
    ...
    if (!versionsById.has(undo.versionId)) {
      issues.push(
        createIssue("error", "UNDO_VERSION_NOT_FOUND", "Undo.version_id 必须指向现有 version", {
          path: getUndoPath(undo.id),
          details: {
            undoId: undo.id,
    ...
    if (undo.originVersionId.trim().length === 0) {
      ...
      issues.push(
        createIssue(
          "error",
          "UNDO_ORIGIN_VERSION_NOT_FOUND",
          "Undo.origin_version_id 必须指向现有 version",
          {
            path: getUndoPath(undo.id),
            details: {
              undoId: undo.id,
    ...
    if (undo.preferredResolutionVersionId.trim().length === 0) {
      issues.push(
        createIssue(
          "error",
          "UNDO_PREFERRED_RESOLUTION_VERSION_MISSING",
          "Undo 必须包含 preferred_resolution_version_id",
          {
            path: getUndoPath(undo.id),
            details: {
              undoId: undo.id
      ...
      issues.push(
        createIssue(
          ...
          "Undo.preferred_resolution_version_id 必须指向现有 version",
          {
            path: getUndoPath(undo.id),
            details: {
              undoId: undo.id,
    ...
    if (undo.carriedForwardAt === null && undo.carriedForwardToVersionId !== null) {
      issues.push(
        createIssue(
          "error",
          "UNDO_CARRIED_FORWARD_TIMESTAMP_MISSING",
          "Undo.carried_forward_to_version_id 存在时必须同时提供 carried_forward_at",
          {
            path: getUndoPath(undo.id),
            details: {
              undoId: undo.id,
    ...
    if (undo.carriedForwardAt !== null && undo.carriedForwardToVersionId === null) {
      issues.push(
        createIssue(
          "error",
          "UNDO_CARRIED_FORWARD_TARGET_MISSING",
          "Undo.carried_forward_at 存在时必须同时提供 carried_forward_to_version_id",
          {
            path: getUndoPath(undo.id),
            details: {
              undoId: undo.id,
    ...
    if (
      ...
      issues.push(
        createIssue(
          "error",
          "UNDO_CARRIED_FORWARD_TARGET_NOT_FOUND",
          "Undo.carried_forward_to_version_id 必须指向现有 version",
          {
            path: getUndoPath(undo.id),
            details: {
              undoId: undo.id,
  ...
  for (const deferredItem of snapshot.deferredItems) {
    const invariantViolations = collectDeferredItemInvariantViolations(
      deferredItem,
      snapshot.todos
    );
    const deferredPath = getDeferredItemPath(
      typeof deferredItem.id === "string" ? deferredItem.id : "invalid"
    );

    for (const violation of invariantViolations) {
      issues.push(
        createIssue("error", violation.code, violation.message, {
          path: deferredPath,
          details: {
            deferredItemId:
              typeof deferredItem.id === "string" ? deferredItem.id : null
          }
        })
      );
    }
    ...
    if (
      invariantViolations.some(
        (violation) => violation.code === "DEFERRED_SHAPE_INVALID"
      )
    ) {
      continue;
    }
    ...
    if (!workItemsById.has(deferredItem.workItemId)) {
      issues.push(
        createIssue(
          ...
          "DeferredItem.work_item_id 必须指向现有 WorkItem",
          {
            path: deferredPath,
            details: {
    ...
    if (!versionsById.has(deferredItem.originVersionId)) {
      issues.push(
        createIssue(
          ...
          "DeferredItem.origin_version_id 必须指向现有 version",
          {
            path: deferredPath,
            details: {
              deferredItemId: deferredItem.id,
    ...
    if (!versionsById.has(deferredItem.targetReviewVersionId)) {
      issues.push(
        createIssue(
          ...
          "DeferredItem.target_review_version_id 必须指向现有 version",
          {
            path: deferredPath,
            details: {
              deferredItemId: deferredItem.id,
  ...
  for (const constraint of snapshot.constraints) {
    ...
    const constraintPath = getConstraintPath(
      typeof constraint.id === "string" ? constraint.id : "invalid"
    );
    ...
    for (const violation of invariantViolations) {
      issues.push(
        createIssue("error", violation.code, violation.message, {
          ...
          details: {
            constraintId:
              typeof constraint.id === "string" ? constraint.id : null
          }
        })
      );
    }
    ...
    if (
      invariantViolations.some(
        (violation) => violation.code === "CONSTRAINT_SHAPE_INVALID"
      )
    ) {
      continue;
    }
    ...
    if (
      ...
      issues.push(
        createIssue(
          ...
          "version-scoped Constraint 必须指向现有 version",
          {
            path: constraintPath,
            details: {
  ...
  for (const asset of snapshot.assets) {
    ...
    if (asset.pathBase !== "project_root") {
      issues.push(
        createIssue("error", "ASSET_PATH_BASE_UNSUPPORTED", "Asset.path_base 只允许 project_root", {
          path: assetPath,
          details: {
            assetId: asset.id,
    ...
    asset.pathHistory.forEach((entry, index) => {
      if (entry.pathBase !== "project_root") {
        issues.push(
          createIssue(
            "error",
            "ASSET_PATH_HISTORY_BASE_UNSUPPORTED",
            "Asset.path_history 只允许 project_root",
            {
              path: assetPath,
              details: {
                assetId: asset.id,
      ...
      validateAssetRelativePath(
        ...
        assetPath,
        entry.relativePath,
        `path_history[${index}].relative_path`
      );
    });
    ...
    for (const workItemId of asset.workItemIds) {
      if (!workItemsById.has(workItemId)) {
        issues.push(
          createIssue("error", "ASSET_WORK_ITEM_NOT_FOUND", "Asset.work_item_ids 必须指向现有 WorkItem", {
            path: assetPath,
            details: {
              assetId: asset.id,
              workItemId
  ...
  for (const event of snapshot.events) {
    validateTransitionTarget(
      ...
      constraintsById,
      assetsById,
      pendingOperationsById,
      approvalArtifactsById
    );
  }
      ...
      if (artifact === undefined) {
        issues.push(
          createIssue(
            ...
            "PendingOperation.approval_artifact_id 必须指向现有 approval artifact",
            {
        ...
        issues.push(
          createIssue(
            ...
            "PendingOperation 与 ApprovalArtifact 的 project/action/target/digest 必须匹配",
            {
              path: getPendingOperationPath(operation.id),
              details: {
                pendingOperationId: operation.id,
    ...
    if (operation === undefined) {
      issues.push(
        createIssue(
          ...
          "ApprovalArtifact.pending_operation_id 必须指向现有 pending operation",
          {
    ...
    if (
      ...
      issues.push(
        createIssue(
          ...
          "ApprovalArtifact 与 PendingOperation 的 project/action/target/digest 必须匹配",
          {
            path: getApprovalArtifactPath(artifact.id),
            details: {
              approvalArtifactId: artifact.id,
  ...
  const operationEventKeys = new Map<string, TransitionEvent>();
  ...
  for (const event of snapshot.events) {
    ...
    if (previous !== undefined) {
      issues.push(
        createIssue(
          "error",
          "OPERATION_EVENT_SEQ_DUPLICATE",
          "同一 operation_id + operation_seq 不应重复",
          {
            path: getEventPath(event),
            details: {
              eventId: event.id,
...
export const validateRouteLedgerJsonDocuments = (
  ...
  for (const document of documents) {
    ...
    const contractValid = validateDocumentContract(
      issues,
      document.path,
      parsed.value!
    );
  ...
  if (projectRecord === undefined) {
    issues.push(
      createIssue("error", "JSON_DOCUMENT_MISSING", "缺少 .routeledger/project.json", {
        path: PROJECT_DOCUMENT_PATH
      })
    );
  }
  ...
  if (currentRefRecord === undefined) {
    issues.push(
      createIssue("error", "JSON_DOCUMENT_MISSING", "缺少 .routeledger/refs/current.json", {
        path: CURRENT_REF_DOCUMENT_PATH
      })
    );
  }

  const currentRef: CurrentRefSnapshot | undefined =
    currentRefRecord === undefined
      ? undefined
      : {
          path: CURRENT_REF_DOCUMENT_PATH,
  ...
  const projectCurrentVersionId =
    projectRecord === undefined ? undefined : asNullableString(projectRecord.current_version_id);

  if (projectRecord !== undefined && projectId === undefined) {
    issues.push(
      createIssue("error", "JSON_DOCUMENT_INVALID", "project.json 缺少有效 id", {
        path: PROJECT_DOCUMENT_PATH
      })
    );
  }

  const syntheticCurrentRefDocument =
    projectId === undefined
      ? undefined
      : {
          path: CURRENT_REF_DOCUMENT_PATH,
          content: `${JSON.stringify(
            {
              schema_version:
                typeof projectRecord?.schema_version === "number"
  ...
  if (projectRecord !== undefined && syntheticCurrentRefDocument !== undefined) {
    try {
      snapshot = decodeProjectAggregateFromJsonDocumentsForValidation(
        [
          ...knownDocuments.filter(
            (document) =>
              document.path !== CURRENT_REF_DOCUMENT_PATH && parsedDocuments.has(document.path)
          ),
      ...
      issues.push(
        createIssue("error", "JSON_DECODE_FAILED", "JSON 文档集无法还原为 aggregate snapshot", {
          details: {
            error: error instanceof Error ? error.message : String(error)
          }
        })
  ...
  if (snapshot !== undefined) {
    const operationsById = new Map(
      snapshot.pendingOperations.map((operation) => [operation.id, operation] as const)
    );
    const legacyAuditArtifactIds = new Set<string>();
      ...
      if (isImmutableCommittedAudit) {
        ...
        issues.push(
          createIssue(
            ...
            "Legacy authorization provenance is audit-only and valid only for an immutable committed operation",
            {
              path: getApprovalArtifactPath(artifact.id),

...
| RL-DOM-003 | WorkItem is a supporting Project-aggregate lineage identity, not a user work surface. Todo, Deferred, and readable legacy Undo records retain that ID across transitions; active WorkItems have exactly one current Todo/Deferred child and closed WorkItems have none, while retained legacy wait Undo remains audit/gate history. The legacy pointer-to-Undo form remains readable and validated separately. | `cap-route-work-semantics.md`; `architecture/work-item-lineage-identity.md`; canonical Todo, Deferred, Undo, and WorkItem schemas | `WorkItem`; `validateWorkItemLineage`; Todo/Deferred operations in `RouteLedgerService`; aggregate validation | `work-item-service.test.ts`; `workflow-service.test.ts`; `json-validate.test.ts`; `sqlite-storage-adapter.test.ts` | confirmed |
...
| RL-ROUTE-006 | `next_action` is a deterministic shared recommendation with blocker precedence; its next Version is the persisted direct sibling successor, never a flattened child-tree neighbor, and stable record order is not business priority. | `cap-mcp-route-operations.md` | current-context/next-action query functions; `RouteLedgerService.getNextAction` | `service-next-action.test.ts`; `mcp-versions.test.ts` | confirmed |
| RL-ROUTE-007 | Every non-close residual destination references an already materialized, actionable Todo, Deferred, or Constraint; close validates the link and never implies an uncreated handoff record. | `cap-mcp-route-operations.md`; residual-audit input schema | `evaluateCloseGate`; `ResidualAuditItem.destinationRecordId` | `gate-service.test.ts`; `json-codec.test.ts` | confirmed |
| RL-ROUTE-008 | A closed Version's closeout summary restores the reviewed residual audit from the committed proposal that completed its close boundary. | `cap-mcp-route-operations.md`; pending-operation schema | `resolveCloseoutResidualAudit`; `collectVersionCloseoutView` | `service-closeout.test.ts` | confirmed |

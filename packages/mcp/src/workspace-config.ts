...
export interface RouteLedgerWorkspaceConfig {
  version: number;
  dataDir: string;
}
...
export interface WorkspaceConfigResolution {
  ...
  sqliteDbPath: string;
  config: RouteLedgerWorkspaceConfig | null;
...
export const getWorkspaceConfigPath = (workspaceRoot: string): string =>
  path.join(getWorkspaceConfigDirectory(workspaceRoot), WORKSPACE_CONFIG_FILENAME);
...
const buildResolution = (
  ...
  dataRoot: string,
  config: RouteLedgerWorkspaceConfig | null,
  status: WorkspaceConfigResolution["status"],
  diagnostics: WorkspaceConfigDiagnostic[]
...
const writeWorkspaceConfig = (workspaceRoot: string, dataDir: string): void => {
  ...
  if (!fs.existsSync(configPath)) {
    if (options.autoCreate === true) {
      writeWorkspaceConfig(
        workspaceRoot,
        options.defaultDataDir ?? DEFAULT_WORKSPACE_DATA_DIR
      );
    } else {
      return buildResolution(
        workspaceRoot,

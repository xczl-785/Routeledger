...
const assertAbsolutePath = (value, fieldName) => {
    ...
    if (!path.isAbsolute(value)) {
        throw new CodexProjectConfigError("ABSOLUTE_PATH_REQUIRED", `${fieldName} must be an absolute path. Received: ${value}`);
    }
...
const assertNonEmpty = (value, fieldName) => {
    ...
    if (trimmed.length === 0) {
        throw new CodexProjectConfigError("EMPTY_VALUE", `${fieldName} must be a non-empty string.`);
    }
    return trimmed;
};
const assertServerName = (serverName) => {
    const normalized = assertNonEmpty(serverName, "serverName");
    if (!/^[A-Za-z0-9_-]+$/u.test(normalized)) {
        throw new CodexProjectConfigError("INVALID_SERVER_NAME", "serverName may only contain letters, digits, underscore, or hyphen.");
    }
    return normalized;
};
...
const normalizeInput = (input) => {
    ...
    const routeledgerRoot = assertAbsolutePath(input.routeledgerRoot, "routeledgerRoot");
    const serverName = assertServerName(input.serverName ?? DEFAULT_SERVER_NAME);
    const hostProfile = assertNonEmpty(input.hostProfile ?? DEFAULT_PROFILE, "hostProfile");
    const actor = {
        id: assertNonEmpty(input.actor?.id ?? DEFAULT_ACTOR.id, "actor.id"),
        displayName: assertNonEmpty(input.actor?.displayName ?? DEFAULT_ACTOR.displayName, "actor.displayName")
    };
    const approver = {
        id: assertNonEmpty(input.approver?.id ?? DEFAULT_APPROVER.id, "approver.id"),
        displayName: assertNonEmpty(input.approver?.displayName ?? DEFAULT_APPROVER.displayName, "approver.displayName")
    };
    ...
    if (!Number.isInteger(startupTimeoutSec) ||
        ...
        toolTimeoutSec <= 0) {
        throw new CodexProjectConfigError("INVALID_TIMEOUT", "startupTimeoutSec and toolTimeoutSec must be positive integers.");
    }
    const relativeRouteLedgerRoot = path.relative(workspaceRoot, routeledgerRoot);
    if (relativeRouteLedgerRoot.length > 0 &&
        (relativeRouteLedgerRoot.startsWith("..") || path.isAbsolute(relativeRouteLedgerRoot))) {
        throw new CodexProjectConfigError("ROUTELEDGER_ROOT_OUTSIDE_WORKSPACE", "routeledgerRoot must stay within workspaceRoot.");
    }
    let source;
    if (input.source.kind === "workspace") {
        source = {
            kind: "workspace",
            routeLedgerWorkspaceRoot: assertAbsolutePath(input.source.routeLedgerWorkspaceRoot, "source.routeLedgerWorkspaceRoot"),
            command: assertNonEmpty(input.source.command ?? "pnpm", "source.command"),
            packageFilter: assertNonEmpty(input.source.packageFilter ?? DEFAULT_MCP_PACKAGE_FILTER, "source.packageFilter"),
            entryScript: assertNonEmpty(input.source.entryScript ?? DEFAULT_MCP_ENTRY_SCRIPT, "source.entryScript")
        };
    }
    else {
        source = {
            kind: "installed-package",
            installRoot: assertAbsolutePath(input.source.installRoot, "source.installRoot"),
            binPath: assertAbsolutePath(input.source.binPath, "source.binPath"),
            command: assertNonEmpty(input.source.command ?? "node", "source.command")
        };
    }
    return {
        workspaceRoot,
...
export const renderCodexProjectConfig = (input) => {
    ...
    return [
        ...
        `default_tools_approval_mode = ${quoteTomlString(normalized.defaultToolsApprovalMode)}`,
        "",
...
export const renderCodexGlobalConfig = (input) => {
    ...
    const filteredArgs = commandShape.args.filter((value, index, array) => !((value === "--workspace-root" || value === "--routeledger-root") &&
...
const writeFileIfChanged = async (filePath, content, overwrite) => {
    const existing = await readFileIfExists(filePath);
    if (existing === content) {
        return false;
    }
    if (existing !== null && !overwrite) {
        throw new CodexProjectConfigError("CONFIG_FILE_EXISTS", `Refusing to overwrite existing file without explicit permission: ${filePath}`);
    }
    await fs.mkdir(path.dirname(filePath), { recursive: true });
    await fs.writeFile(filePath, content, "utf8");
    return true;
};
const detectWriteKind = (filePath) => path.basename(filePath) === "config.toml" ? "project-config" : "fragment";
const getDefaultFragmentPath = (workspaceRoot, serverName) => path.join(workspaceRoot, ".codex", `${serverName}${DEFAULT_FRAGMENT_SUFFIX}`);
export const writeCodexProjectConfig = async (input) => {
    const normalized = normalizeInput(input);
    const content = renderCodexProjectConfig(normalized);
    const explicitOutputPath = input.outputPath === undefined
        ? undefined
        : assertAbsolutePath(input.outputPath, "outputPath");
    const strategy = input.existingConfigStrategy ?? "write-fragment";
    const defaultConfigPath = path.join(normalized.workspaceRoot, ".codex", "config.toml");
    if (explicitOutputPath !== undefined) {
        const created = await writeFileIfChanged(explicitOutputPath, content, strategy === "overwrite");
        return {
    ...
    if (existingConfig === null || strategy === "overwrite") {
        const created = await writeFileIfChanged(defaultConfigPath, content, strategy === "overwrite");
        return {
    ...
    if (strategy === "error") {
        throw new CodexProjectConfigError("CONFIG_FILE_EXISTS", `Codex project config already exists: ${defaultConfigPath}`);
    }
    const fragmentPath = getDefaultFragmentPath(normalized.workspaceRoot, normalized.serverName);
...
export const planCodexProjectConfigWrite = async (input) => {
    const normalized = normalizeInput(input);
    const content = renderCodexProjectConfig(normalized);
    const explicitOutputPath = input.outputPath === undefined
        ? undefined

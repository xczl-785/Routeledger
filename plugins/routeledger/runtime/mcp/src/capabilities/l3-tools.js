...
import { ApplicationError, BALANCED_ALWAYS_PROMPT_ACTIONS, BALANCED_AUTO_ACTIONS, createExactProposalDecisionRequest, digestL3AuthorizationPolicy, digestL3AuthorizationProfile, orchestrateL3Operation, projectDecisionArtifact } from "../../../core/src/index.js";
...
export const createL3AuthorizationTools = (dependencies) => {
    const { service, approver, hostProfile, initialBinding, options, usesCodexNativeToolAdmission, digestAuthorizationPath, digestRouteLedgerRoot } = dependencies;
    return [
        ...
        }), { title: "Get L3 Authorization Status", riskLevel: "read-only" }, async (input) => {
        ...
        defineTool("recommend_l3_authorization_profile", { what: "Build a conservative V3 profile candidate.", warning: "candidate only" }, objectSchema({
            ...
            expiresInHours: integerSchema("Delegated policy lifetime in hours. Defaults to 24.", { minimum: 1, maximum: 168 }),
            decisionBudget: integerSchema("Maximum exact decisions allowed by the standing policy. Defaults to 16.", { minimum: 1, maximum: 100 }),
            maxAuthorizationTtlSeconds: integerSchema("Maximum exact-authorization validity. Defaults to 3600 seconds.", { minimum: 30, maximum: 86400 })
        }, ["projectId", "mode"]), { title: "Recommend L3 Authorization Profile", riskLevel: "read-only" }, async (input) => {
...
export const createL3ProposalTools = (dependencies) => {
    const { service, approver, hostProfile, initialBinding, options, digestRouteLedgerRoot } = dependencies;
    return [
        defineTool("recommend_l3_authorization_policy", { what: "Build a bound conservative L3 policy candidate." }, objectSchema({
            projectId: stringSchema("RouteLedger project ID."),
            expiresInHours: integerSchema("Policy lifetime in hours. Defaults to 24.", { minimum: 1, maximum: 168 }),
            decisionBudget: integerSchema("Maximum exact decisions per standing-policy rule. Defaults to 16.", { minimum: 1, maximum: 100 })
        }, ["projectId"]), { title: "Recommend L3 Authorization Policy", riskLevel: "read-only" }, async (input) => {
        ...
        defineTool("list_l3_proposals", { what: "List pending and historical L3 proposals." }, objectSchema({ projectId: stringSchema("RouteLedger project ID.") }, [
            "projectId"
        ]), { title: "List L3 Proposals", riskLevel: "read-only" }, async (input) => ({
            ok: true,
        ...
        }, ["projectId", "pendingOperationId"]), { title: "Get L3 Proposal", riskLevel: "read-only" }, async (input) => ({
            ok: true,
            data: await service.getL3Proposal(input.projectId, input.pendingOperationId)
        }))
    ];
};
const createL3DecisionAdapter = (dependencies, proposal) => {
    const { service, approver, hostProfile, initialBinding, options, usesCodexNativeToolAdmission, digestRouteLedgerRoot } = dependencies;
...
const executeExistingL3Proposal = async (dependencies, proposal, idempotencyKey, debugToolName = "execute_l3_operation") => {
...
export const createL3OperationTools = (dependencies) => {
    ...
    const admittedProposalExecutions = new Map();
    const assertExecutionControlPlaneAvailable = () => {
        if (options.l3Authorization === undefined) {
    ...
    return [
        defineTool("propose_l3_operation", {
        ...
        }, ["projectId", "actionType", "targetId", "reason"]), { title: "Propose L3 Operation", riskLevel: "write" }, async (input) => ({
            ok: true,
        ...
        defineTool("execute_l3_operation", {
            ...
            if (entry.inFlight === undefined) {
                entry.inFlight = executeExistingL3Proposal(dependencies, proposal, input.idempotencyKey);
            }
        ...
        defineTool("execute_admitted_proposal", {
            ...
            expectedOperationDigest: stringSchema("Optional exact proposal digest returned by the proposal call.")
            ...
            if (input.expectedOperationDigest !== undefined &&
                input.expectedOperationDigest !== proposal.digest.value) {
                throw new InvalidToolInputError("expectedOperationDigest does not match the persisted proposal", {
            ...
            if (inFlight === undefined) {
                inFlight = executeExistingL3Proposal(dependencies, proposal, proposal.id, "execute_admitted_proposal");
        ...
        }, ["projectId", "pendingOperationId"]), { title: "Approve L3 Operation", riskLevel: "high-risk" }, async (input) => {
            const proposal = await service.getL3Proposal(input.projectId, input.pendingOperationId);
        ...
        defineTool("commit_l3_operation", {
        ...
        }, ["projectId", "pendingOperationId", "reason"]), { title: "Reject L3 Operation", riskLevel: "high-risk" }, async (input) => ({
            ok: true,

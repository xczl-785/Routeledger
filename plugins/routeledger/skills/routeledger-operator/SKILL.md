...
- Read: `inspect_runtime`, `inspect_route`.
...
5. Before every write/high-risk route operation, use the returned RouteLedger root assertion. This includes `dry_run` operations on `propose_route_change` and `execute_route_change`. They are binding-sensitive operations, not read-only MCP tools. Do not continue until the returned binding matches the intended project.

...
- [x] 第 1 项：控制面本地化边界与中文 Agent 展示层（不扩展完整国际化；`0.10.9` 增加机器可读作用域）
- [x] 第 2 项：compact 响应体积和重复信息（已随 `0.10.7` 发布）
- [x] 第 3 项：更短的 Agent 快速路径（六组首次概念已随 `0.10.8` 发布）
- [ ] 第 4 项：隔离测试/低风险个人项目的轻量授权模式
- [x] 第 5 项：首个 Deferred 的下游 Version 可执行引导（`0.10.9` 三步依赖计划）
- [x] 第 6 项：agent_only 初始化响应降噪（`0.10.9` 省略人类文档建议）

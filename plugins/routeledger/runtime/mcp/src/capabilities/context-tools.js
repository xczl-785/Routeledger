...
const getCurrentContextOutputSchema = toolOutputSchema(objectSchema({
    ...sharedContextFields,
    versions: flexibleObjectArrayOutputSchema,
    legacyUndo: flexibleObjectArrayOutputSchema
}, [...sharedContextRequired, "versions"]));
const nextActionContextOutputSchema = toolOutputSchema(objectSchema(sharedContextFields, sharedContextRequired));
...
const planVersionCloseoutOutputSchema = toolOutputSchema(objectSchema({
...
export const createContextTools = (dependencies) => {
    ...
    return [
        ...
        defineTool("next_action", { what: "Read the shared next route action." }, objectSchema({ projectId: stringSchema("RouteLedger project ID.") }, ["projectId"]), {
            title: "Next Action",
            riskLevel: "read-only",
        ...
        }, ["projectId", "entryFiles"]), { title: "Check Doc Drift", riskLevel: "read-only" }, withInputAdapter(adaptCheckDocDriftInput, async (input) => {
        ...
        }, ["projectId"]), { title: "Summarize Version Closeout", riskLevel: "read-only" }, async (input) => {
        ...
        defineTool("plan_version_closeout", { what: "Plan concrete steps to clear a version closeout." }, objectSchema({
            projectId: stringSchema("RouteLedger project ID."),
            versionId: stringSchema("Optional version ID. Defaults to project.currentVersionId."),
            eventLimit: integerSchema("How many related recent events to include in the embedded summary.", { minimum: 1, maximum: 50 })
        }, ["projectId"]), {
        ...
        }, ["projectId"]), { title: "List Versions Window", riskLevel: "read-only" }, withInputAdapter(adaptListVersionsWindowInput, async (input) => {
        ...
        defineTool("list_versions", { what: "List project versions." }, objectSchema({ projectId: stringSchema("RouteLedger project ID.") }, ["projectId"]), { title: "List Versions", riskLevel: "read-only" }, async (input) => ({ ok: true, data: await service.listVersions(input.projectId) })),
        defineTool("check_start_gate", { what: "Evaluate a version start gate." }, objectSchema({
            projectId: stringSchema("RouteLedger project ID."),
            versionId: stringSchema("Target version ID.")
        }, ["projectId", "versionId"]), { title: "Check Start Gate", riskLevel: "read-only" }, async (input) => {
        ...
        }, ["projectId", "versionId"]), { title: "Check Close Gate", riskLevel: "read-only" }, async (input) => {
        ...
        }, ["projectId"]), { title: "Get Version Structure", riskLevel: "read-only" }, async (input) => {
        ...
        defineTool("get_version_transition_guide", {
        ...
        }, ["projectId", "targetVersionId"]), { title: "Get Version Transition Guide", riskLevel: "read-only" }, async (input) => {

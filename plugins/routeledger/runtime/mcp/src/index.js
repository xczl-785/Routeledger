...
import { ApplicationError, BALANCED_ALWAYS_PROMPT_ACTIONS, BALANCED_AUTO_ACTIONS, BATCH_CREATE_VERSIONS_MODES, BATCH_PREVIOUS_CURRENT_POLICIES, DomainError, createExactProposalDecisionRequest, digestL3AuthorizationPolicy, digestL3AuthorizationProfile, orchestrateL3Operation, projectDecisionArtifact, ROUTE_OPERATION_WORKFLOW_MODES, RouteLedgerService, isBatchCreateVersionsMode, isBatchPreviousCurrentPolicy, isRouteOperationWorkflowMode } from "../../core/src/index.js";
...
export const createRouteLedgerMcpRegistry = (options) => {
    ...
    const createExistingL3DecisionAdapter = (proposal) => {
        if (hostProfile === "codex" && options.hostPermissionContext?.status === "unavailable") {
            throw new ApplicationError("AUTHORIZATION_CONTROL_PLANE_UNAVAILABLE", options.hostPermissionContext.reason, { reason: options.hostPermissionContext.code, pendingOperationId: proposal.id });
        }
        ...
        defineTool("get_l3_authorization_status", { what: "Inspect active L3 authorization." }, objectSchema({
            ...
            const effectiveMode = options.hostPermissionContext?.status === "resolved"
                ? {
                    ...
                    profileCompatible: profile === undefined ? null : profile.mode === options.hostPermissionContext.mode
                }

...
import { buildVersionStructureView, RouteLedgerQueryService } from "../../../core/src/index.js";
import { PROJECT_DOCUMENT_PATH, loadValidatedProjectAggregateFromJsonDirectory } from "../../../json/src/index.js";
class ReadOnlySnapshotStorage {
    snapshot;
...
const versionSort = (left, right) => left.order - right.order;
const shutdownStateReasonPrefix = "shutdown:";
const isShutdownStateReason = (stateReason) => typeof stateReason === "string" && stateReason.startsWith(shutdownStateReasonPrefix);
const describeVersionState = (version) => {
...
const isUndoCarriedForwardAwayFromVersion = (undo, versionId) => undo.carriedForwardAt !== null &&
...
const isUndoBlockingCloseForVersion = (undo, versionId) => undo.status === "wait" && !isUndoCarriedForwardAwayFromVersion(undo, versionId);
const createBindingSummary = (input) => {
...
const describeUnusedSqliteReadModel = (routeledgerRoot) => {
...
const buildStorageSummary = (options) => ({
    ...
    canonicalJson: {
        ...
        projectPath: options.binding.routeledgerRoot === null
...
const toProposalSummary = (proposal) => ({
    id: proposal.id,
...
const toAuditEventSummary = (event) => ({
    id: event.id,
...
const getBlockingUndosForVersion = (undos, versionId) => dedupeLegacyUndos(undos.filter((undo) => (undo.versionId === versionId ||
    undo.originVersionId === versionId ||
    undo.preferredResolutionVersionId === versionId) &&
    isUndoBlockingCloseForVersion(undo, versionId)));
const summarizeTreeNode = (version) => {
    if (version === null) {
        return null;
    }
    return {
...
export const buildMissionControlViewModel = async (snapshot, binding, storage, screen, message) => {
    ...
    const childCountByVersionId = new Map();
    const openTodoCountByVersionId = new Map();
    const pendingDeferredCountByVersionId = new Map();
    const activeConstraintCountByVersionId = new Map();
    for (const version of versions) {
        childCountByVersionId.set(version.id, versions.filter((candidate) => candidate.parentVersionId === version.id).length);
        openTodoCountByVersionId.set(version.id, snapshot.todos.filter((todo) => todo.versionId === version.id && (todo.status === "wait" || todo.status === "running")).length);
        pendingDeferredCountByVersionId.set(version.id, snapshot.deferredItems.filter((deferred) => deferred.targetReviewVersionId === version.id && deferred.status === "pending").length);
        activeConstraintCountByVersionId.set(version.id, snapshot.constraints.filter((constraint) => constraint.status === "active" &&
            constraint.scope.type === "version" &&
            constraint.scope.versionId === version.id).length);
    }
    ...
    const treeSource = currentVersionId === null
        ? null
    ...
    const buildVersionPanel = (version) => {
        const versionTodos = snapshot.todos
            .filter((todo) => todo.versionId === version.id &&
        ...
        const versionDeferred = snapshot.deferredItems
            .filter((deferred) => deferred.status === "pending" &&
            (deferred.originVersionId === version.id ||
                deferred.targetReviewVersionId === version.id))
            .sort((left, right) => {
            const dueOrder = Number(dueDeferredIdSet.has(right.id)) - Number(dueDeferredIdSet.has(left.id));
            ...
            targetReviewVersionTitle: versionsById.get(deferred.targetReviewVersionId)?.title ??
                deferred.targetReviewVersionId,
            isDue: dueDeferredIdSet.has(deferred.id),
            updatedAt: deferred.updatedAt
        }));
        const versionConstraints = snapshot.constraints
            ...
            scope: constraint.scope.type === "project" ? "project" : "current_version",
        ...
        return {
            ...
            previousVersionTitle: version.previousVersionId === null
                ? null
                : versionsById.get(version.previousVersionId)?.title ?? null,
            nextVersionTitle: version.nextVersionId === null
                ? null
                : versionsById.get(version.nextVersionId)?.title ?? null,
            parentVersionTitle: version.parentVersionId === null
                ? null
    ...
    const openLegacyRecords = dedupeLegacyUndos(snapshot.undos.filter((undo) => undo.status === "wait"));
    const currentVersionLegacyBlockers = currentVersionId === null
        ? []
        : getBlockingUndosForVersion(snapshot.undos, currentVersionId);
    const roadmap = contextData.versions.map((summary) => {
    ...
    const tree = treeSource === null
        ...
        : {
            ...
            siblings: treeSource.siblings
                ...
                .map((version) => summarizeTreeNode({
                ...version,
                description: versionsById.get(version.id)?.description ?? "",
                displayLabel: version.displayLabel
            })),
            children: treeSource.childVersions.map((version) => summarizeTreeNode({
                ...version,
                description: versionsById.get(version.id)?.description ?? "",
                displayLabel: version.displayLabel
            }))
    ...
    return {
        ...
        identity: {
            ...
            currentVersionDisplayLabel: currentVersion === null ? null : describeVersionState(currentVersion).displayLabel,
            currentVersionStateReason: currentVersion === null ? null : describeVersionState(currentVersion).stateReason,
            currentVersionOrder: currentVersion?.order ?? null,
            versionCount: versions.length,
            updatedAt: snapshot.project.updatedAt
        },
        overview: {
            openTodoCount: snapshot.todos.filter((todo) => todo.status === "wait" || todo.status === "running").length,
            pendingDeferredCount: snapshot.deferredItems.filter((deferred) => deferred.status === "pending").length,
            dueDeferredCount: contextData.dueDeferredIds.length,
            activeConstraintCount: snapshot.constraints.filter((constraint) => constraint.status === "active").length,
        ...
        nextAction: contextData.nextAction === null
            ? null
            : {
...
export const buildMissionControlResponse = async (input) => {
    const binding = createBindingSummary(input);
    const sqlite = binding.routeledgerRoot === null
        ? {
            status: "absent",
            dbPath: "",
            error: null
    ...
    try {
        ...
        const screen = loaded.snapshot.project.currentVersionId === null ? "current_closed" : "ready";
        ...
        return buildMissionControlViewModel(loaded.snapshot, binding, storage, screen, screen === "current_closed"
            ? routeIsEmpty

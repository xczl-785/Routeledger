...
export const digestL3AuthorizationProfile = (profile) => crypto
    .createHash("sha256")
...
export const validateL3AuthorizationProfile = (profile) => {
    const issues = [];
    ...
    for (const field of bindingFields) {
        if (!isNonEmptyString(profile.binding?.[field])) {
            addIssue(issues, "PROFILE_BINDING_REQUIRED", `$.binding.${field}`, `${field} is required`);
        }
    }
    if (profile.binding?.trustedClientId !== null &&
        !isNonEmptyString(profile.binding?.trustedClientId)) {
        addIssue(issues, "TRUSTED_CLIENT_ID_INVALID", "$.binding.trustedClientId", "trustedClientId must be null or a non-empty string");
    }
    if (!["interactive", "delegated", "preauthorized"].includes(profile.mode)) {
    ...
    if (profile.mode === "delegated") {
        if (profile.delegatedPolicy === null) {
            ...
            if (policyBinding.projectId !== profile.binding.projectId ||
                ...
                addIssue(issues, "DELEGATED_POLICY_BINDING_MISMATCH", "$.delegatedPolicy.binding", "the delegated policy must exactly match the profile binding");
            }
        }
    }
    ...
    if (isIsoDate(profile.createdAt) && isIsoDate(profile.updatedAt)) {
        if (Date.parse(profile.updatedAt) < Date.parse(profile.createdAt)) {
            addIssue(issues, "PROFILE_TIME_ORDER_INVALID", "$.updatedAt", "updatedAt must not precede createdAt");
        }
    }
    if (profile.adoptedFrom !== undefined) {
        if (profile.adoptedFrom.schemaVersion !== 1 ||
            ...
            addIssue(issues, "ADOPTION_SOURCE_INVALID", "$.adoptedFrom", "adoptedFrom must contain exact v1 provenance");
        }
    }
    if (isNonEmptyString(profile.profileDigest)) {
        if (profile.profileDigest !== digestL3AuthorizationProfile(profile)) {
            addIssue(issues, "PROFILE_DIGEST_MISMATCH", "$.profileDigest", "profileDigest does not match the authorization-effective profile");
        }
    }
    else {

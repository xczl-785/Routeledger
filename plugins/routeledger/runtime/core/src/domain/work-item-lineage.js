 ...
 */
export const validateWorkItemLineage = (workItem, todos, undos, deferredItems = []) => {
    ...
    // they remain in `wait`.
    const currentActiveChildren = [];
    for (const todo of todos) {
        if (todo.workItemId !== workItem.id) {
            continue;
        }
        if (todo.projectId !== workItem.projectId) {
            throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "Todo.workItemId 必须指向同一 Project aggregate 内的 WorkItem", { workItemId: workItem.id, todoId: todo.id });
        }
    ...
    for (const undo of undos) {
        ...
        if (undo.projectId !== workItem.projectId) {
            throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "Undo.workItemId 必须指向同一 Project aggregate 内的 WorkItem", { workItemId: workItem.id, undoId: undo.id });
        }
    }
    for (const deferredItem of deferredItems) {
        ...
        if (deferredItem.projectId !== workItem.projectId) {
            throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "DeferredItem.workItemId 必须指向同一 Project aggregate 内的 WorkItem", { workItemId: workItem.id, deferredItemId: deferredItem.id });
        }
    ...
    if (workItem.status === "closed") {
        ...
        if (currentActiveChildren.length !== 0) {
            throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "closed WorkItem 不应保留 active 子记录", { workItemId: workItem.id, activeChildren: currentActiveChildren });
        }
        return;
    }
    if (workItem.activeRecordType === null || workItem.activeRecordId === null) {
    ...
    if (workItem.activeRecordType === "undo") {
        const legacyActiveUndo = undos.find((undo) => undo.id === workItem.activeRecordId &&
            undo.workItemId === workItem.id &&
            undo.projectId === workItem.projectId &&
            undo.status === "wait");
        if (currentActiveChildren.length !== 0 || legacyActiveUndo === undefined) {
            throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "legacy Undo 指针必须指向同 Project 的 wait Undo，且不得同时存在 current Todo/Deferred", {
    ...
    if (currentActiveChildren.length !== 1) {
        throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "active WorkItem 必须恰有一个 active 子记录", { workItemId: workItem.id, activeChildren: currentActiveChildren });
    }
    const [activeChild] = currentActiveChildren;
    if (activeChild.type !== workItem.activeRecordType ||
        activeChild.id !== workItem.activeRecordId) {
        throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "WorkItem active 指针必须指向唯一的 active 子记录", {
            workItemId: workItem.id,

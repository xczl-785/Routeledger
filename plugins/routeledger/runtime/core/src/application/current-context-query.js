...
const estimateBytes = (value) => Buffer.byteLength(stableStringify(value), "utf8");
const sanitizeLegacyGateDetails = (gate) => {
    if (gate === null || typeof gate !== "object") {
        return gate;
    }
    const sanitized = structuredClone(gate);
        ...
        sanitized.blockers = sanitized.blockers.map((blocker) => {
            if (blocker !== null &&
                ...
                return {
                    ...
                    message: "A legacy work record still affects this gate. Use get_current_context(includeLegacyUndo=true) for audit details.",
                    recordIds: Array.isArray(blocker.recordIds)
                        ? blocker.recordIds
                        : []
                };
            }
...
const sanitizeStatusRisksForAgent = (risks) => risks.map((risk) => risk.code === "OPEN_UNDOS_BLOCK_CLOSE"
    ? {
        ...risk,
        code: "LEGACY_WORK_BLOCKS_CLOSE",
        summary: "当前 version 存在仍会阻塞 close gate 的历史工作记录；需要显式打开 legacy audit 后处理。"
    }
    : risk);
const clampContextBudget = (budgetBytes, project) => {
...
const clampVersionWindowSize = (value, fallback) => {
...
const summarizeConstraint = (constraint) => ({
    id: constraint.id,
...
const buildBlockingRiskCodes = (statusRisks) => statusRisks
...
const buildStatusRisks = (options) => {
    const risks = [];
    const { currentVersion, nextVersion, currentVersionOpenTodos, currentVersionOpenUndos, pendingProposals, diagnosticVersions, runningVersionPointerDrift, startGate, closeGate } = options;
    ...
    if (startGate !== null &&
        !startGate.allowed &&
        startGate.blockers.some((blocker) => blocker.code !== "DUE_DEFERRED_REQUIRES_REVIEW")) {
        risks.push({
            code: "START_GATE_BLOCKED",
            severity: "blocking",
            summary: "目标 version 的 start gate 仍有 blocker；需要先审计并处理，再尝试启动。",
            recordIds: startGate.blockers
                .filter((blocker) => blocker.code !== "DUE_DEFERRED_REQUIRES_REVIEW")
...
const buildNextAction = (options) => {
    ...
    if (currentVersion?.state === "complete" &&
        ...
        return {
            actionType: deferredIds.length > 0 ? "review_deferred" : "review_context",
            summary: deferredIds.length > 0
            ...
            requiresL3Approval: false,
            recordIds: deferredIds.length > 0
                ? deferredIds
    ...
    if (currentVersion?.state === "close" && nextVersion?.state === "ready") {
        ...
        if (startGate !== null && !startGate.allowed) {
            return {
                ...
                requiresL3Approval: false,
                recordIds: startGate.blockers.flatMap((blocker) => blocker.recordIds),
...
export const buildDerivedCurrentContextData = (snapshot, options = {}) => {
    ...
    const runningVersionPointerDrift = {
        uniqueRunningVersion: currentVersion !== null &&
    ...
    const openTodos = snapshot.todos
        ...
        .slice()
        .sort((left, right) => {
        const orderDifference = (versionOrderById.get(left.versionId) ?? Number.MAX_SAFE_INTEGER) -
    ...
    const currentVersionOpenTodos = currentVersion === null ? [] : openTodos.filter((todo) => todo.versionId === currentVersion.id);
    const currentVersionOpenUndos = currentVersion === null
        ? []
        : openUndos.filter((undo) => (undo.versionId === currentVersion.id ||
    ...
    const startTargetVersion = currentVersion?.state === "ready"
        ? currentVersion
        : nextVersion?.state === "ready"
            ? nextVersion
            : null;
    const startGate = startTargetVersion === null
        ? null
        : {
            ...evaluateStartGate({
...
export const buildVersionsWindowResult = (snapshot, input) => {
    const versions = snapshot.versions.slice().sort((left, right) => left.order - right.order);
...
export const buildCurrentContextResult = (snapshot, input) => {
    ...
    const omittedCounts = {};
    const data = structuredClone(baseContext);
    ...
    if (estimateBytes(data) > budgetBytes) {
        if (data.currentVersion !== null && typeof data.currentVersion === "object") {
            delete data.currentVersion.description;
    ...
    if (estimateBytes(data) > budgetBytes) {
        ...
        const legacyUndoArray = (data.legacyUndo ?? []);
        const proposalsArray = data.pendingL3Proposals;
    ...
    return {
        ...
        meta: {
            budgetBytes,
            payloadBytes,
            truncated: payloadBytes > budgetBytes ||
...
export const buildNextActionResult = (snapshot, input) => {

...
} from "../domain/transition-event.js";
...
import {
  buildAuthorizationCommitClaimId,
  getExactArtifactReceiptBinding,
  hasV2AuthorizationProfile,
...
export interface L3OperationCommitUseCases {
  commitL3Operation(input: CommitL3OperationInput): Promise<{
...
type L3OperationCommitStorage = ProjectSnapshotReader & ProjectSnapshotWriter;

const replaceRecord = <T extends { id: string }>(
  records: T[],
  nextRecord: T,
): T[] =>
...
const createAuditEvents = (
  ...
  createTransitionEvents(
    drafts,
    { projectId, actor, now, operationId },
    deps.idGenerator,
    { startSeq },
  );
...
const requireApprovalArtifact = (
  ...
  const artifact = snapshot.approvalArtifacts.find(
    (item) => item.id === approvalArtifactId,
  );
...
const hasEmptyExtendedGateState = (gateSnapshot: GateSnapshot): boolean => {
  ...
  const isNew = (code: string): boolean =>
  ...
  if (gateSnapshot.kind === "start") {
    return (
      empty(gateSnapshot.dueDeferredIds) &&
  ...
  if (gateSnapshot.kind === "close") {
    return (
      empty(gateSnapshot.unresolvedDeferredIds) &&
      empty(gateSnapshot.blockedConstraintIds) &&
      !gateSnapshot.blockers.some((blocker) => isNew(blocker.code))
    );
  }
  if (gateSnapshot.kind === "shutdown") {
    return (
      empty(gateSnapshot.ordinaryCloseGate.unresolvedDeferredIds) &&
      empty(gateSnapshot.ordinaryCloseGate.blockedConstraintIds) &&
      !gateSnapshot.blockers.some((blocker) => isNew(blocker.code)) &&
      !gateSnapshot.ordinaryCloseGate.blockers.some((blocker) =>
        isNew(blocker.code),
      )
    );
  }
...
const summarizeGate = (gateSnapshot: GateSnapshot) => ({
...
export class L3OperationCommitService implements L3OperationCommitUseCases {
  constructor(
    private readonly options: {
      storage: L3OperationCommitStorage;
      deps: DomainDependencies;
      securityPort: L3ProposalSecurityPort;
      controlPlane: TrustedL3AuthorizationControlPlane | null;
      commitCoordinator: ExactCommitCoordinator | null;
    },
  ) {}

  async commitL3Operation(input: CommitL3OperationInput) {
    const ownership =
      this.options.commitCoordinator === null
        ? null
    ...
    try {
      ...
      if (token !== null) await this.options.commitCoordinator!.release(token);
    }
  }
  ...
  private async commitOwned(
    ...
    const snapshot = await loadRequiredProjectAggregate(
      this.options.storage,
      input.projectId,
    );
    const pendingOperation = requirePendingOperation(
      snapshot,
    ...
    if (
      input.approvalArtifactId === undefined ||
    ...
    const artifact = requireApprovalArtifact(
      snapshot,
    ...
    if (
      ...
      (exactReceiptBinding === null ||
        !(await this.options.controlPlane.exactStore.verifyReceipt(
          exactReceiptBinding,
        )))
    )
      throw new ApplicationError(
    ...
    if (new Date(artifact.expiresAt).getTime() <= new Date(now).getTime()) {
      ...
      throw new ApplicationError(
        ...
        { approvalArtifactId: artifact.id, expiresAt: artifact.expiresAt },
      );
    }
    ...
    const claim =
      exactReceiptBinding === null || this.options.controlPlane === null
        ? null
        : await this.options.controlPlane.exactStore.claimCommit(
            exactReceiptBinding,
            {
              claimId: buildAuthorizationCommitClaimId(
                artifact,
                pendingOperation,
              ),
              claimedAt: now,
    ...
    const applied = this.applyCommittedOperation(
      snapshot,
      pendingOperation,
      liveDescription,
      { actor: input.actor, now, operationId: context.operationId },
    );
    ...
    if (
      ...
      const finalized =
        await this.options.controlPlane.exactStore.finalizeCommit(
          exactReceiptBinding,
  ...
  private assertArtifact(
    ...
    const mismatch = (
      ...
      if (condition) throw new ApplicationError(code, message, details);
    };
    mismatch(
    ...
    mismatch(
      artifact.consumedAt !== null || artifact.status === "consumed",
      "APPROVAL_ARTIFACT_ALREADY_CONSUMED",
      "approval artifact 已被消费",
      { approvalArtifactId: artifact.id, consumedAt: artifact.consumedAt },
    );
    ...
    mismatch(
      ...
      { approvalArtifactId: artifact.id, status: artifact.status },
    );
    mismatch(
  ...
  private applyCommittedOperation(
    ...
    live: L3ProposalSecurityDescription,
    context: { actor: Actor; now: string; operationId: string },
  ): { snapshot: ProjectAggregateSnapshot; events: TransitionEvent[] } {
    const startGate =
      live.gateSnapshot.kind === "start"
        ? {
    ...
    switch (pending.actionType) {
      case "start_version": {
        const started = startVersionDomain(
          requireVersion(snapshot, pending.targetId),
          startGate,
          context,
          this.options.deps,
        );
      ...
      case "reopen_version": {
        const reopened = reopenVersionDomain(
          requireVersion(snapshot, pending.targetId),
          context,
          this.options.deps,
        );
      ...
      case "reorder_versions": {
        if (
          pending.actionType === "insert_version" &&
        ...
        const setCurrent =
          pending.actionType === "create_version" &&
      ...
      default: {
        const exhaustive: never = pending.actionType;
        throw new ApplicationError(
          "ACTION_NOT_IMPLEMENTED",
          "未支持的 L3 action",
          { actionType: exhaustive },
        );
      }
    }
  }
  ...
  private applyBatch(
    ...
    if (plan === undefined || plan.length === 0)
      throw new ApplicationError(
        ...
        "batch_create_versions 缺少 normalized plan",
      );
    let next: ProjectAggregateSnapshot = {
      ...snapshot,
    ...
    if (
      payload.batchSetCurrentTo !== undefined &&
      payload.batchSetCurrentTo !== null
    ) {
      const target =
        ids.get(payload.batchSetCurrentTo) ??

...
当前推荐通过 Codex 插件使用 RouteLedger。已发布稳定版为 0.10.0，由不可变标签 `routeledger-plugin-v0.10.0` 固定；它提供按任务与风险拆分的公开工具表面、统一的英文 agent 协议，以及由用户明确选择的项目内容语言。`main` 是唯一发布干线，`codex-marketplace` 只保留为 0.3.3 的历史锚点分支：
...
- 安装：当前只通过 Codex 插件（Git marketplace）分发；已发布稳定版为 0.10.0；
...
- [Release policy](docs/release/release-policy.md) 与 [0.10.0 release note](docs/release/release-notes/0.10.0.md) — 当前已发布基线与发布流程

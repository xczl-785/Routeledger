...
export const buildAuthorizationCommitClaimId = (artifact, pendingOperation) => `commit_${crypto
    .createHash("sha256")
    .update(`${artifact.id}\0${pendingOperation.id}\0${pendingOperation.digest.value}`, "utf8")
    .digest("hex")}`;
export const buildExactAuthorizationBinding = (pendingOperation, routeledgerRootDigest) => ({
...
export const buildExactReceiptBinding = (candidate, artifactId) => ({
...
export const hasV2AuthorizationProfile = (artifact) => artifact.profileId !== undefined &&
    artifact.modeEpoch !== undefined &&
    artifact.profileDigest !== undefined;
export const getExactArtifactReceiptBinding = async (input) => {
    if (input.controlPlane === null || input.artifact.authorizationId === undefined)
        return null;
    const expected = buildExactAuthorizationBinding(input.pendingOperation, input.controlPlane.routeledgerRootDigest);
...
export class L3ExactAuthorizationService {
    options;
    constructor(options) {
    ...
    async authorizeL3Operation(input) {
        ...
        const binding = buildExactAuthorizationBinding(pendingOperation, controlPlane.routeledgerRootDigest);

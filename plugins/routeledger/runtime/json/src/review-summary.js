...
const toMap = (records) => new Map(records.map((record) => [record.id, record]));
const sortByTitle = (records) => [...records].sort((left, right) => left.title.localeCompare(right.title, "en") || left.id.localeCompare(right.id, "en"));
const takeTopItems = (records) => records.slice(0, MAX_LIST_ITEMS);
...
const isTodoOpen = (todo) => todo.status === "wait" || todo.status === "running";
const isUndoOpen = (undo) => undo.status === "wait";
const isDeferredPending = (deferred) => deferred.status === "pending";
const isVersionReopened = (fromState, toState) => ["close", "complete", "suspend"].includes(fromState) &&
    ["wait", "ready", "running"].includes(toState);
const summarizeVersionRef = (snapshot, versionId) => {
    if (versionId === null) {
        return {
            versionId: null,
            title: null
...
const buildWarnings = (headSnapshot) => {
    const warnings = [];
    const pendingOperations = headSnapshot.pendingOperations.filter((operation) => operation.status === "pending");
    if (pendingOperations.length > 0) {
        warnings.push(`head 存在 ${pendingOperations.length} 个 pending proposal: ${pendingOperations
    ...
    if (openTodos.length > 0) {
        warnings.push(`head 仍有 ${openTodos.length} 个 open todo: ${openTodos
            .slice(0, MAX_LIST_ITEMS)
            .map((todo) => todo.title)
            .join(", ")}`);
    }
    ...
    if (pendingDeferred.length > 0) {
        warnings.push(`head 仍有 ${pendingDeferred.length} 个 Deferred 等待复评: ${pendingDeferred
            .slice(0, MAX_LIST_ITEMS)
    ...
    if (openUndos.length > 0) {
        warnings.push(`Legacy compatibility: head 仍有 ${openUndos.length} 个 open Undo audit record: ${openUndos
            .slice(0, MAX_LIST_ITEMS)
            .map((undo) => undo.title)
            .join(", ")}`);
    }
    ...
    if (runningVersions.length > 1) {
        warnings.push(`head 存在 ${runningVersions.length} 个 running version: ${runningVersions
            .slice(0, MAX_LIST_ITEMS)
    ...
    if (runningVersions.length === 1 && headSnapshot.project.currentVersionId !== null) {
        ...
        const currentVersion = headSnapshot.versions.find((version) => version.id === headSnapshot.project.currentVersionId);
        if (currentVersion !== undefined && currentVersion.id !== runningVersion.id) {
            warnings.push(`head current pointer drift: current=${currentVersion.title} (${currentVersion.id}), running=${runningVersion.title} (${runningVersion.id})`);
        }
    }
    const selfReferentialUndos = headSnapshot.undos.filter((undo) => undo.status === "wait" &&
        undo.versionId === undo.preferredResolutionVersionId);
    if (selfReferentialUndos.length > 0) {
        warnings.push(`Legacy compatibility: head 存在 ${selfReferentialUndos.length} 个 self-referential Undo blocker: ${selfReferentialUndos
            .slice(0, MAX_LIST_ITEMS)
            .map((undo) => undo.title)
            .join(", ")}`);
    }
...
const buildSummaryText = (summary, labels) => {
    ...
    if (!summary.overview.hasSemanticChanges) {
        ...
        if (summary.versions.createdCount > 0 || summary.versions.stateChangedCount > 0) {
            lines.push(`Versions: +${summary.versions.createdCount} created, ${summary.versions.stateChangedCount} state changes, ${summary.versions.reopenedCount} reopened.`);
        }
        if (summary.todos.createdCount > 0 ||
            summary.todos.closedCount > 0 ||
            summary.todos.convertedCount > 0) {
            lines.push(`Todos: +${summary.todos.createdCount} created, ${summary.todos.closedCount} closed, ${summary.todos.convertedCount} converted.`);
        }
        if (summary.deferred.createdCount > 0 ||
            summary.deferred.statusChangedCount > 0 ||
            summary.deferred.reviewTargetChangedCount > 0) {
            lines.push(`Deferred: +${summary.deferred.createdCount} created, ${summary.deferred.statusChangedCount} status changes, ${summary.deferred.reviewTargetChangedCount} review-target changes.`);
        }
        if (summary.constraints.createdCount > 0 ||
            summary.constraints.retiredCount > 0) {
            lines.push(`Constraints: +${summary.constraints.createdCount} created, ${summary.constraints.retiredCount} retired.`);
        }
        const legacyUndos = summary.legacyCompatibility.undos;
        if (legacyUndos.createdCount > 0 ||
            legacyUndos.closedCount > 0 ||
            legacyUndos.reassignedCount > 0 ||
            legacyUndos.convertedCount > 0) {
            lines.push(`Legacy compatibility - Undo audit records: +${legacyUndos.createdCount} created, ${legacyUndos.closedCount} closed, ${legacyUndos.reassignedCount} reassigned, ${legacyUndos.convertedCount} converted.`);
        }
        if (summary.pendingOperations.proposedCount > 0 ||
            summary.pendingOperations.committedCount > 0 ||
            summary.pendingOperations.rejectedCount > 0) {
            lines.push(`Pending operations: ${summary.pendingOperations.proposedCount} proposed, ${summary.pendingOperations.committedCount} committed, ${summary.pendingOperations.rejectedCount} rejected in range.`);
        }
        if (summary.approvalArtifacts.approvedCount > 0 ||
            summary.approvalArtifacts.consumedCount > 0) {
            lines.push(`Approval artifacts: ${summary.approvalArtifacts.approvedCount} approved, ${summary.approvalArtifacts.consumedCount} consumed in range.`);
        }
        if (summary.assets.createdCount > 0 ||
            summary.assets.statusChangedCount > 0 ||
            summary.assets.pathChangedCount > 0) {
            lines.push(`Assets: +${summary.assets.createdCount} created, ${summary.assets.statusChangedCount} status changes, ${summary.assets.pathChangedCount} path changes.`);
        }
    }
    lines.push(`Events added: ${summary.eventsDigest.totalAdded} (${Object.entries(summary.eventsDigest.byType)
...
export const buildProjectAggregateReviewSummary = (baseSnapshot, headSnapshot, options = {}) => {
    if (baseSnapshot.project.id !== headSnapshot.project.id) {
        throw new RouteLedgerJsonReviewSummaryError("PROJECT_ID_MISMATCH", "两个 ref 的 projectId 不一致，无法做语义 review summary", {
    ...
    const createdVersions = takeTopItems(headSnapshot.versions
    ...
    const changedVersionStates = takeTopItems(sortByTitle(headSnapshot.versions
        .filter((version) => baseVersions.has(version.id))
        .map((version) => ({
        version,
    ...
    const reopenedVersions = changedVersionStates.filter((change) => isVersionReopened(change.fromState, change.toState));
    ...
    const createdTodos = takeTopItems(sortByTitle(headSnapshot.todos.filter((todo) => !baseTodos.has(todo.id))).map((todo) => ({
        id: todo.id,
        title: todo.title
    })));
    const todoStatusChanges = takeTopItems(sortByTitle(headSnapshot.todos
    ...
    const convertedTodos = todoStatusChanges
        ...
        .map(({ id, title }) => ({ id, title }));
    const reassignedTodos = takeTopItems(sortByTitle(headSnapshot.todos
        .filter((todo) => baseTodos.has(todo.id))
        .map((todo) => ({
        todo,
        baseTodo: baseTodos.get(todo.id)
    ...
    const createdDeferred = takeTopItems(sortByTitle(headSnapshot.deferredItems.filter((deferred) => !baseDeferred.has(deferred.id))).map((deferred) => ({
        id: deferred.id,
        title: deferred.title,
        status: deferred.status,
        targetReviewVersionId: deferred.targetReviewVersionId
    })));
    const deferredStatusChanges = takeTopItems(sortByTitle(headSnapshot.deferredItems
        .filter((deferred) => baseDeferred.has(deferred.id))
        .map((deferred) => ({
        deferred,
        baseDeferred: baseDeferred.get(deferred.id)
    }))
        .filter(({ deferred, baseDeferred: baseRecord }) => deferred.status !== baseRecord.status)
    ...
    const deferredReviewTargetChanges = takeTopItems(sortByTitle(headSnapshot.deferredItems
        .filter((deferred) => baseDeferred.has(deferred.id))
        .map((deferred) => ({
        deferred,
        baseDeferred: baseDeferred.get(deferred.id)
    }))
        .filter(({ deferred, baseDeferred: baseRecord }) => deferred.targetReviewVersionId !== baseRecord.targetReviewVersionId)
        .map(({ deferred, baseDeferred: baseRecord }) => ({
        id: deferred.id,
        title: deferred.title,
    ...
    const createdConstraints = takeTopItems(headSnapshot.constraints
    ...
    const constraintStatusChanges = takeTopItems(headSnapshot.constraints
        .filter((constraint) => baseConstraints.has(constraint.id))
        .map((constraint) => ({
        constraint,
        baseConstraint: baseConstraints.get(constraint.id)
    }))
        .filter(({ constraint, baseConstraint }) => constraint.status !== baseConstraint.status)
        .sort((left, right) => left.constraint.rule.localeCompare(right.constraint.rule, "en") ||
    ...
    const createdUndos = takeTopItems(sortByTitle(headSnapshot.undos.filter((undo) => !baseUndos.has(undo.id))).map((undo) => ({
        id: undo.id,
        title: undo.title
    })));
    const undoStatusChanges = takeTopItems(sortByTitle(headSnapshot.undos
    ...
    const convertedUndos = undoStatusChanges
        ...
        .map(({ id, title }) => ({ id, title }));
    const reassignedUndos = takeTopItems(sortByTitle(headSnapshot.undos
        .filter((undo) => baseUndos.has(undo.id))
        .map((undo) => ({
        undo,
        baseUndo: baseUndos.get(undo.id)
    }))
        .filter(({ undo, baseUndo }) => undo.preferredResolutionVersionId !== baseUndo.preferredResolutionVersionId)
        .map(({ undo, baseUndo }) => ({
        id: undo.id,
        title: undo.title,
    ...
    const createdPending = headSnapshot.pendingOperations.filter((operation) => !basePendingOperations.has(operation.id));
    const pendingStatusChanges = takeTopItems(headSnapshot.pendingOperations
        ...
        .sort((left, right) => left.operation.createdAt.localeCompare(right.operation.createdAt, "en") ||
    ...
    const createdApprovalArtifacts = headSnapshot.approvalArtifacts.filter((artifact) => !baseApprovalArtifacts.has(artifact.id));
    const approvalStatusChanges = takeTopItems(headSnapshot.approvalArtifacts
        ...
        .sort((left, right) => left.artifact.createdAt.localeCompare(right.artifact.createdAt, "en") ||
    ...
    const createdAssets = takeTopItems(headSnapshot.assets
    ...
    const assetStatusChanges = takeTopItems(headSnapshot.assets
        .filter((asset) => baseAssets.has(asset.id))
        .map((asset) => ({
        asset,
        ...
        .sort((left, right) => left.asset.relativePath.localeCompare(right.asset.relativePath, "en"))
        .map(({ asset, baseAsset }) => ({
        id: asset.id,
        relativePath: asset.relativePath,
    ...
    const assetPathChanges = takeTopItems(headSnapshot.assets
        .filter((asset) => baseAssets.has(asset.id))
        .map((asset) => ({
        asset,
        baseAsset: baseAssets.get(asset.id)
    ...
    const partialSummary = {
        ...
        deferred: {
            createdCount: headSnapshot.deferredItems.filter((deferred) => !baseDeferred.has(deferred.id)).length,
        ...
        constraints: {
            createdCount: headSnapshot.constraints.filter((constraint) => !baseConstraints.has(constraint.id)).length,
            ...
            retiredCount: headSnapshot.constraints.filter((constraint) => {
                ...
                return (baseConstraint !== undefined &&

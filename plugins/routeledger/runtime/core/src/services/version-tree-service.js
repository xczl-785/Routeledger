...
const buildOrderedTree = (versions) => {
    ...
    const unorderedByParent = new Map();
    for (const version of versions) {
...
const requireExistingVersion = (tree, versionId, field) => {
...
const requireNewTargetId = (tree, targetId) => {
...
const assertAnchorIsMutable = (version, field) => {
...
const normalizeSiblingInsert = (options) => {
    ...
    const previousIndex = previousVersionId === null
        ? -1
        : siblings.findIndex((version) => version.id === previousVersionId);
    const nextIndex = nextVersionId === null ? -1 : siblings.findIndex((version) => version.id === nextVersionId);
...
const normalizeCreateVersionPayload = (tree, targetId, payload) => {
...
const normalizeInsertVersionPayload = (tree, targetId, payload) => {
    requireNewTargetId(tree, targetId);
    const previous = payload.previousVersionId === undefined || payload.previousVersionId === null
        ? null
        : requireExistingVersion(tree, payload.previousVersionId, "previousVersionId");
    const next = payload.nextVersionId === undefined || payload.nextVersionId === null
        ? null
...
const normalizeCreateChildVersionPayload = (tree, targetId, payload) => {
    requireNewTargetId(tree, targetId);
    ...
    if ((payload.previousVersionId === undefined || payload.previousVersionId === null) &&
...
const normalizeReorderPayload = (tree, targetId, payload) => {
    ...
    if (targetVersion.previousVersionId !== null) {
        assertAnchorIsMutable(requireExistingVersion(tree, targetVersion.previousVersionId, "previousVersionId"), "previousVersionId");
    }
    if (targetVersion.nextVersionId !== null) {
        assertAnchorIsMutable(requireExistingVersion(tree, targetVersion.nextVersionId, "nextVersionId"), "nextVersionId");
    }
    const previous = payload.previousVersionId === undefined || payload.previousVersionId === null
        ? null
        : requireExistingVersion(tree, payload.previousVersionId, "previousVersionId");
...
export const normalizeVersionTreePayload = (input) => {
    ...
    switch (input.actionType) {
        ...
        default: {
            const exhaustiveActionType = input.actionType;
            return exhaustiveActionType;
        }
    }
};
const insertSibling = (siblings, version, previousVersionId, nextVersionId) => {
...
const rebuildVersions = (originalVersions, orderedByParent, now) => {
    const originalMap = new Map(originalVersions.map((version) => [version.id, version]));
    const rebuilt = new Map();
    let order = 1;
    const visitParent = (parentVersionId) => {
        ...
        for (let index = 0; index < siblings.length; index += 1) {
            ...
            const treeUpdated = original === undefined ||
...
const createTreeChangeMetadata = (before, after) => ({
    before: {
...
const buildEventDrafts = (beforeVersions, afterVersions, actionType) => {
...
export const applyVersionTreeMutation = (input) => {
    ...
    const tree = buildOrderedTree(input.versions);
    const orderedByParent = new Map([...tree.orderedByParent.entries()].map(([parentVersionId, siblings]) => [
        parentVersionId,
        siblings.slice()
    ]));
    switch (input.actionType) {
        ...
        case "create_child_version": {
            ...
            orderedByParent.set(parentVersionId, insertSibling(siblings, createdVersion, normalizedPayload.previousVersionId ?? null, normalizedPayload.nextVersionId ?? null));
            break;
        }
        case "reorder_versions": {
            ...
            const siblings = (orderedByParent.get(parentVersionId) ?? []).filter((version) => version.id !== targetVersion.id);
            orderedByParent.set(parentVersionId, insertSibling(siblings, targetVersion, normalizedPayload.previousVersionId ?? null, normalizedPayload.nextVersionId ?? null));
            break;
        }
        default: {
            const exhaustiveActionType = input.actionType;

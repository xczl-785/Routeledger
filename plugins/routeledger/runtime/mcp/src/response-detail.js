...
const compactRuntimeData = (value, omittedSections) => {
    if (!isRecord(value))
        return null;
...
const compactRuntimeContext = (value, path, omittedSections) => {
    const binding = isRecord(value.binding) ? value.binding : null;
    const activeProject = isRecord(value.activeProject) ? value.activeProject : null;
...
const compactEvent = (value, path, omittedSections) => {
    if (!isRecord(value))
        return value;
    ...
    return Object.fromEntries([...retainedKeys]
...
const compactRouteRecord = (value, path, omittedSections) => {
    const compact = {};
...
const compactProjectSummary = (value) => {
    if (!isRecord(value))
        return value;
    return definedEntries([
        ["id", value.id],
...
const compactVersionSummary = (value) => {
...
const compactTodoSummary = (value) => {
    if (!isRecord(value))
        return value;
    return definedEntries([
        ["id", value.id],
...
const compactIdempotency = (value) => {
    if (!isRecord(value))
        return value;
    return definedEntries([
...
const compactTodoWriteData = (value, omittedSections) => {
    if (!isRecord(value))
        return null;
    if (value.workItem !== undefined)
        omittedSections.add("data.workItem");
    return definedEntries([
...
const compactDeferredSummary = (value) => {
    if (!isRecord(value))
        return value;
    return definedEntries([
        ["id", value.id],
...
const compactConstraintSummary = (value) => {
    if (!isRecord(value))
        return value;
    return definedEntries([
        ["id", value.id],
...
const mapArray = (value, projector) => Array.isArray(value) ? value.map(projector) : value;
const mapNonEmptyArray = (value, projector) => Array.isArray(value) && value.length === 0
    ? undefined
    : mapArray(value, projector);
const compactNextActionData = (value, omittedSections) => {
    if (!isRecord(value))
        return null;
    if (Object.keys(value).some((key) => !["project", "currentVersion", "nextVersion", "nextAction"].includes(key))) {
...
const compactCurrentContextData = (value, omittedSections) => {
    if (!isRecord(value))
        return null;
    ...
    const routeWindow = Array.isArray(value.versions)
        ? value.versions.filter((item) => !isRecord(item) ||
    ...
    return definedEntries([
        ...
        [
            ...
            Array.isArray(value.pendingL3Proposals) && value.pendingL3Proposals.length > 0
                ? value.pendingL3Proposals.map((item) => isRecord(item)
...
const compactOperationMeta = (value, context, omittedSections) => {
    if (!isRecord(value))
        return value;
    const runtimeContext = isRecord(value.runtimeContext)
        ...
        : undefined;
    const isFocusedRead = (context.toolName === "inspect_runtime" && context.operation === "runtime") ||
...
const projectCompactValue = (value, path, omittedSections, parentKey = "") => {
    if (Array.isArray(value)) {
        const source = parentKey === "events"
    ...
    if (["proposal", "pendingOperation", "rejection", "approvalArtifact"].includes(parentKey)) {
...
const collectEntityIds = (value, ids) => {
    if (ids.size >= 16)
        return;
    if (Array.isArray(value)) {
...
const inferDeltaKind = (operation, riskLevel) => {
...
const buildCompactData = (data, context, omittedSections) => {
    ...
    const status = [record.status, record.state, record.mode].find((value) => typeof value === "string") ??
        "succeeded";
    const primaryId = [record.proposalId, record.pendingOperationId, record.id, record.targetId]
...
export const parseAgentResponseDetail = (value) => value === "compact" || value === "standard" || value === "audit" ? value : null;
export const applyAgentResponseDetail = (response, context) => {
    if (context.detail === "standard" && !context.explicit)
        return response;

...
export const L3_AUTHORIZATION_POLICY_SCHEMA_VERSION = 1;
...
const actionSet = new Set(L3_AUTHORIZATION_ACTIONS);
const modeSet = new Set(["interactive", "delegated", "preauthorized"]);
const relationSet = new Set(["current", "legal-successor", "other"]);
...
export const digestL3AuthorizationPolicy = (policy) => crypto.createHash("sha256").update(JSON.stringify(canonicalize(policy))).digest("hex");
const addIssue = (issues, code, path, message) => issues.push({ code, path, message });
const validateActions = (value, path, issues) => {
...
export const validateL3AuthorizationPolicy = (policy) => {
    const issues = [];
    ...
    if (!isNonEmptyString(policy.binding?.routeledgerRootDigest)) {
        addIssue(issues, "ROOT_BINDING_REQUIRED", "$.binding.routeledgerRootDigest", "RouteLedger root digest binding is required");
    }
    if (policy.defaultEffect !== "prompt" && policy.defaultEffect !== "deny") {
        addIssue(issues, "DEFAULT_EFFECT_INVALID", "$.defaultEffect", "defaultEffect must be prompt or deny");
    }
    if (!Array.isArray(policy.rules)) {
        ...
        for (const [index, rule] of policy.rules.entries()) {
            ...
            if (rule.resources?.targetIds !== undefined) {
                if (!Array.isArray(rule.resources.targetIds) ||
                    rule.resources.targetIds.length === 0 ||
                    rule.resources.targetIds.some((targetId) => !isNonEmptyString(targetId))) {
                    addIssue(issues, "TARGET_IDS_INVALID", `${base}.resources.targetIds`, "targetIds must be a non-empty string array when provided");
                }
            }
            ...
            if (rule.conditions?.allowedTargetRelations !== undefined) {
                if (!Array.isArray(rule.conditions.allowedTargetRelations) ||
                    ...
                    addIssue(issues, "TARGET_RELATIONS_INVALID", `${base}.conditions.allowedTargetRelations`, "allowedTargetRelations contains an invalid value");
                }
            }
            if (rule.conditions?.expiresAt !== undefined &&
    ...
    if (!Array.isArray(policy.alwaysPrompt)) {
        ...
        for (const [index, action] of policy.alwaysPrompt.entries()) {
            if (!actionSet.has(action)) {
                addIssue(issues, "ALWAYS_PROMPT_ACTION_INVALID", `$.alwaysPrompt[${index}]`, "unknown L3 action");
            }
        }
    }
    return { valid: issues.length === 0, issues };
};
const bindingMatches = (binding, context) => binding.projectId === context.projectId &&
...
const ruleMatches = (rule, context) => {
    ...
    if (rule.conditions?.gateMustPass === true && !context.gateAllowed)
        return false;
    if (rule.conditions?.allowedTargetRelations !== undefined &&
        !rule.conditions.allowedTargetRelations.includes(context.targetRelation)) {
        return false;
    }
    if (rule.conditions?.requiredCurrentVersionId !== undefined &&
...
export const evaluateL3AuthorizationPolicy = (policy, context) => {
    ...
    const denyRule = policy.rules.find((rule) => rule.effect === "deny" && ruleMatches(rule, context));
...
export const buildBalancedL3AuthorizationPolicy = (input) => ({

...
const MISSION_CONTROL_NOTICE_MESSAGES: Record<
  string,
  { en: (accessUrl: string | null) => string; "zh-CN": (accessUrl: string | null) => string }
> = {
  MISSION_CONTROL_RUNNING: {
    en: (accessUrl) =>
      `RouteLedger Mission Control is running. Open it at: ${accessUrl ?? "the reported local address"}`,
    "zh-CN": (accessUrl) =>
      `RouteLedger Mission Control 已启动，可通过以下地址访问：${accessUrl ?? "返回的本地地址"}`
  },
...
const localizeSystemValue = (
  ...
  if (
    ...
    if (localized !== undefined) {
      record.message = localized[locale](
        typeof record.accessUrl === "string" ? record.accessUrl : null
      );
    }
  }
    ...
    : {
        error: (() => {
          ...
          if (typeof localized.code === "string" && typeof localized.message === "string") {
            const details =
              localized.details !== null &&
              typeof localized.details === "object" &&
              !Array.isArray(localized.details)
                ? (localized.details as Record<string, unknown>)
                : null;
            localized.message =

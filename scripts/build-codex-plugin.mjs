...
const buildAttestationLocator = (pluginVersion) => {
...
const renderPluginRuntimeIdentity = ({ runtimePackageVersion, pluginVersion, sourceTreeState, buildCommit, runtimePayloadDigest }) => {
  const attestation = buildAttestationLocator(pluginVersion);
  return (
    ...
    `  attestation: ${JSON.stringify(attestation, null, 2).replaceAll("\n", "\n  ")},\n` +
...
const injectPluginRuntimeIdentity = async (buildProvenance) => {
  ...
  const attestation = buildAttestationLocator(manifest.version);
  await fs.writeFile(
...
const writeReleaseMetadata = async (runtimeIdentity, buildProvenance) => {
  ...
  const releaseMetadata = {
    ...
    runtimeIdentity: {
      ...
      attestation: runtimeIdentity.attestation,
      buildCommit: buildProvenance.buildCommit,
      artifactDigest: null,
    ...
    attestation: {
      ...
      retention: "github-release-asset",
      sourceCommit: null,

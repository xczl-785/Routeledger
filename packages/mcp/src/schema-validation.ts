...
export const validateValueAgainstSchema = (
  ...
  if (Array.isArray(anyOf)) {
    const matchedSchema = anyOf.filter(isObject).some((candidate) =>
      validateValueAgainstSchema(candidate, value, path).length === 0
    );
    if (!matchedSchema) issues.push({ path: formatPath(path), message: "Value does not match any allowed schema." });
    return issues;
  }
  ...
  if (typeof expectedType === "string") {
    ...
    switch (expectedType) {
      case "object": if (!isObject(value)) return [{ path: formatPath(path), message: `Expected object, received ${actualType}.` }]; break;
      case "array": if (!Array.isArray(value)) return [{ path: formatPath(path), message: `Expected array, received ${actualType}.` }]; break;
      case "string": if (typeof value !== "string") return [{ path: formatPath(path), message: `Expected string, received ${actualType}.` }]; break;
      case "integer": if (typeof value !== "number" || !Number.isInteger(value)) return [{ path: formatPath(path), message: `Expected integer, received ${actualType}.` }]; break;
      case "boolean": if (typeof value !== "boolean") return [{ path: formatPath(path), message: `Expected boolean, received ${actualType}.` }]; break;
      case "null": if (value !== null) return [{ path: formatPath(path), message: `Expected null, received ${actualType}.` }]; break;
      default: return [{ path: formatPath(path), message: `Unsupported schema type '${expectedType}'.` }];
    }
  }
  if (Array.isArray(schema.enum) && !hasEnumMatch(schema.enum, value)) issues.push({ path: formatPath(path), message: `Expected one of ${schema.enum.map(String).join(", ")}.` });
  if ("const" in schema && !Object.is(schema.const, value)) issues.push({ path: formatPath(path), message: `Expected constant value ${String(schema.const)}.` });
  ...
  if (typeof schema.minimum === "number" && typeof value === "number" && value < schema.minimum) issues.push({ path: formatPath(path), message: `Expected ${describeExpectedType(schema)} greater than or equal to ${schema.minimum}.` });
  if (typeof schema.maximum === "number" && typeof value === "number" && value > schema.maximum) issues.push({ path: formatPath(path), message: `Expected ${describeExpectedType(schema)} less than or equal to ${schema.maximum}.` });
  return issues;
};

...
const assertReviewInput = (targetReviewVersionId, reason, action) => {
    assertRequired(targetReviewVersionId, "target_review_version_id", action);
    assertRequired(reason, "reason", action);
};
const assertPending = (deferred, action) => {
    if (deferred.status !== "pending") {
        throw new DomainError("INVALID_DEFERRED_TRANSITION", `${action} 仅允许处理 pending Deferred`, {
            deferredId: deferred.id,
            status: deferred.status
        });
    }
};
const assertActivePointer = (workItem, deferred) => {
    if (workItem.status !== "active" ||
        ...
        throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "WorkItem active 指针与 Deferred 不一致", {
...
const assertTodoActivePointer = (workItem, todo) => {
    if (workItem.status !== "active" ||
        ...
        throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "WorkItem active 指针或归属与 Todo 不一致", {
            todoId: todo.id,
...
const createWorkItem = (projectId, title, type, originVersionId, deferredId, actor, now, id) => ({
    id,
    projectId,
    title,
    type,
...
export const createDeferred = ({ projectId, originVersionId, targetReviewVersionId, title, reason, description = "", reviewTrigger = null, actor, deps, workItem, workItemType = "other" }) => {
    ...
    if (workItem?.status === "active") {
        throw new DomainError("INVALID_WORK_ITEM_ACTIVE", "已有 active WorkItem 不能直接创建新的 active Deferred", { workItemId: workItem.id });
    }
    ...
    const nextWorkItem = workItem === undefined
        ? createWorkItem(projectId, title, workItemType, originVersionId, deferredId, actor, context.now, workItemId)
        : {
            ...workItem,
            projectId,
            title,
...
export const deferTodo = ({ todo, workItem, targetReviewVersionId, reason, note, reviewTrigger = null, actor, deps }) => {
    ...
    if (todo.status !== "wait" && todo.status !== "running") {
        throw new DomainError("INVALID_TODO_TRANSITION", "仅 open Todo 可以转为 Deferred", {
            todoId: todo.id,
            status: todo.status
        });
    }
...
export const deferAgain = ({ deferred, workItem, targetReviewVersionId, reason, note, reviewTrigger = deferred.reviewTrigger, actor, deps }) => {
...
export const activateDeferred = ({ deferred, workItem, versionId, reason, note = reason, actor, deps }) => {
...
export const resolveDeferred = ({ deferred, workItem, outcome, reason, note = reason, decisionRef = null, actor, deps }) => {
    if (outcome !== "superseded" &&
        outcome !== "rejected" &&
        outcome !== "out_of_scope") {
        throw new DomainError("INVALID_DEFERRED_TRANSITION", "resolve deferred outcome 仅允许 superseded、rejected 或 out_of_scope", {
            deferredId: deferred.id,
            outcome
        });
    }
    ...
    if ((outcome === "rejected" || outcome === "out_of_scope") &&
        (decisionRef === null || decisionRef.trim().length === 0)) {
        throw new DomainError("MISSING_REQUIRED_FIELD", `${outcome} resolution 必须提供 decision_ref`, {
            deferredId: deferred.id,
            outcome
        });
    }

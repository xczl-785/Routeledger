...
import {
  ...
  type WriteCodexProjectConfigOptions
} from "./index.js";
...
const parseArgs = (
  ...
  mode: "render" | "write";
  options: WriteCodexProjectConfigOptions;
} => {
  ...
  const routeLedgerWorkspaceRoot = getFlagValue(
    argv,
    "--routeledger-workspace-root"
  );
  ...
  if (installRoot !== undefined || binPath !== undefined) {
    if (installRoot === undefined || binPath === undefined) {
      throw new CodexProjectConfigError(
        "SOURCE_FLAGS_INCOMPLETE",
        "--install-root and --bin-path must be provided together."
      );
    }
  ...
  throw new CodexProjectConfigError(
    ...
    "Provide either --routeledger-workspace-root or both --install-root and --bin-path."
  );
};

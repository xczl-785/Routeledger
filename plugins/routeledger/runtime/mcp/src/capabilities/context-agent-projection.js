...
export const sanitizeVersionStructureForAgent = (structure) => {
    ...
    const operations = Array.isArray(sanitized.legalOperations)
        ...
        : [];
    const openUndos = sanitized.openUndos !== null && typeof sanitized.openUndos === "object"
        ? sanitized.openUndos
        : {};
...
export const sanitizeDocDriftForAgent = (result) => {
    ...
    const routeTruth = sanitized.routeTruth !== null && typeof sanitized.routeTruth === "object"
        ...
        : {};
    const openUndoCount = typeof routeTruth.openUndoCount === "number" ? routeTruth.openUndoCount : 0;
    ...
    if (Array.isArray(routeTruth.statusRiskCodes)) {
        const statusRiskCodes = routeTruth.statusRiskCodes.map((code) => code === "OPEN_UNDOS_BLOCK_CLOSE" ? "LEGACY_BLOCKERS_REQUIRE_AUDIT" : code);
        if (openUndoCount > 0) {

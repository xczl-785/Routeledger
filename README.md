...
当前推荐通过 Codex 插件使用 RouteLedger。当前发布版为 0.10.4，由不可变标签 `routeledger-plugin-v0.10.4` 固定；它提供 agent-first 交互配置、可直接执行的推荐动作和可选 compact MCP 响应。`main` 是唯一发布干线，`codex-marketplace` 只保留为 0.3.3 的历史锚点分支：
...
- 安装：当前只通过 Codex 插件（Git marketplace）分发；已发布稳定版为 0.10.4；
...
- [Release policy](docs/release/release-policy.md) 与 [0.10.4 release note](docs/release/release-notes/0.10.4.md) — 当前候选基线与发布流程

export const validateExactAuthorizationCandidate = (input) => {
    const { candidate, request, context, profile, expectedSource, expectedIssuer, expectedPolicyId = null, expectedPolicyDigest = null, now } = input;
    const binding = candidate.binding;
    if (request.projectId !== context.projectId ||
    ...
    if (binding.proposalId !== request.proposalId ||
    ...
    if (candidate.policyId !== expectedPolicyId ||
    ...
    else if (candidate.profileId !== profile.profileId ||

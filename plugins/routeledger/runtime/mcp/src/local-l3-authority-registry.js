...
const assertPrivateFile = async (filePath, label) => {
...
const resolveTrustedRegistryRoot = async (input, create) => {
...
export const buildLocalL3AuthorityBindingKey = (identity) => `b2-${sha256(JSON.stringify(canonicalize(identity)))}`;
const bindingDirectory = (registryRoot, bindingKey) => path.join(registryRoot, "bindings", bindingKey);
const ensureRegistryMarker = async (registryRoot) => {
...
const withBindingLock = async (directory, operation) => {
...
const profileAuthorizationSemantics = (profile) => canonicalize({
...
export const loadLocalL3AuthorityProfileRegistry = async (input) => {
    const registryRoot = await resolveTrustedRegistryRoot(input, false);
    parseMarker(await readJson(path.join(registryRoot, "registry-v2.json"), "Local L3 authority registry marker"));
    return {
...
export const installLocalL3AuthorizationProfile = async (input) => {
    ...
    await withBindingLock(directory, async () => {
        ...
        if (existing !== null) {
            ...
            const semanticsChanged = JSON.stringify(profileAuthorizationSemantics(existing)) !==
            ...
            if (input.profile.modeEpoch !== expectedModeEpoch) {
                throw new Error(semanticsChanged

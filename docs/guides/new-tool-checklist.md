...
1. Register the tool with `defineTool(...)` in `packages/mcp/src/index.ts`.
   Schema validation, risk metadata, `expectedRouteLedgerRoot` injection,
...
3. Add at least one behavior test in
   ...
   root-assertion failure case that verifies zero state change.

...
const getConfigValue = (argv, flagName, envName) => getFlagValue(argv, flagName) ?? process.env[envName];
export const parseSqliteReadModel = (value) => {
    if (value === undefined) {
        return "enabled";
    }
...
export const discoverDefaultLocalL3AuthorityRegistry = async () => {
    const registryRoot = path.join(os.homedir(), ".routeledger", "host-authority", "l3-v2");
    try {
...
export const main = async (argv = process.argv.slice(2)) => {
    ...
    const approverName = getConfigValue(argv, "--approver-name", "ROUTELEDGER_MCP_APPROVER_NAME");

...
| RL-JSON-004 | Canonical replacement is staged and recoverable; write locks and head revision reject active writers and stale snapshots. | `cap-canonical-json-contract.md`; `cap-json-first-runtime-storage.md` | `replaceRouteLedgerJsonDocuments`; `recoverRouteLedgerJsonReplacement`; `acquireRouteLedgerJsonWriteLock`; `attachProjectAggregateHeadRevision`; `JsonFirstStorageAdapter.saveProjectAggregate` | `json-codec.test.ts`; `json-first-storage.test.ts` | confirmed |
...
| RL-BIND-003 | Binding-sensitive writes and high-risk previews require an exact absolute `expectedRouteLedgerRoot` before entering a write path. | `cap-mcp-route-operations.md`; tool input schemas | `runBindingPreflight`; route-tool wrappers in `packages/mcp/src/index.ts` | `mcp-write-guards.test.ts`; `mcp-runtime-context.test.ts`; `mcp-versions.test.ts` | confirmed |
